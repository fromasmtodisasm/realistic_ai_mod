...
void CXVehicleProxy::MoveLikeAHelicopter(SOBJECTSTATE * state)
{
	...
	if(tScale>.1f)		//to prevent stuff when debugging
		tScale = .1f;
	...
	else
	{
		...
		// if state->bodystate==1 - ignore objects below, only terrain is used for altitude calculations
		float	deltaA=UpdateAltitude( m_Movement, state->fValueAux, absMinAlt,
		// if attacking or dropping - don't go lower than target
...
//////////////////////////////////////////////////////////////////////
//returns height of the highest static entity object in self bbox extended down by depth
...
Vec3d CXVehicleProxy::UpdateThreatHeli( void* threat )
{
	...
	if (physEnt && physEnt->GetStatus(&status))
	{
		...
		if(GetLengthSquared(dirHor)<1)	// prefer to move vertically - correction direction is too different from current movement direction
			dir.z *= 50.55f;
...
Vec3d CXVehicleProxy::CarAvoidCollision( float range )
{
	...
	if( obstVehicle )
	{
		...
		if(dotZ>-0.1f)	// moving away from obstacle -- do nothing
			return correction;
