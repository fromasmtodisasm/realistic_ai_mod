...
//
//  History:
//  - Created by Petar Kotevski and Kirill Bulatsev from XPlayer.cpp
//	- October 11, 2002: Major changes to boat/vehicle code by Marco Corbetta
//	- Modified by Filippo De Luca
...
void CPlayer::EnterVehicle( CVehicle *pVehicle, eInVehiclestate state, const char *szHelperName)
{
	...
	// update weapon usage
	// gunner has priority over driver
	// passengers don't use vehicle weapon
...
void CPlayer::LeaveVehicle()
{
	...
	//before enter into the vehicle , so below "ReleaseWeaponUser" we force to select the last weaponid.
...
void CVehicle::SetWeaponUser(int entId)
{
		...
		pCurUserEntity->GetContainer()->QueryContainerInterface(CIT_IPLAYER,(void**)&shooter) )
	{
		...
		{
			// set to gunner angle limits from the weapon
...
////////////////////////////////////////////////////////////////
	...
	// define world position of the target/crosshair, calculate weapon angles
