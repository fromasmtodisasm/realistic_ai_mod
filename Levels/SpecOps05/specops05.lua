...
function Mission:OnInit()
	...
	Language:LoadStringTable("specops05.xml");
	Hud:FlashObjectives({},"");
...
function Mission:Event_M_Need_Boat()
	...
	Hud:PushObjective({},Localize("River_Throw"));
	Hud:PushObjective({},Localize("Find_Boat5"));
	_localplayer.cnt.health=_localplayer.cnt.max_health;
	_localplayer.cnt.armor=_localplayer.cnt.max_armor;
end

function Mission:Event_M_Goto_Link()
	Hud:SetRadarObjective("Radar_Link");
	Hud:AddMessage(Localize("NewObjective"),9);
	Hud:CompleteObjective(Localize("Find_Boat5"));
	Hud:PushObjective({},Localize("Go_To_Comm5"));
end
...
function Mission:Event_MoveToOuter()	
	local DummyDooch=System:GetEntityByName("Dooch_Boat_RELOCATE");	
	local DummyLock=System:GetEntityByName("Lock_Boat_RELOCATE");	
	local DummyDen=System:GetEntityByName("Den_Boat_RELOCATE");
...
function Mission:Event_MoveToPreFight()	
	local DummyDooch=System:GetEntityByName("Dooch_preFight_RELOCATE");	
	local DummyLock=System:GetEntityByName("Lock_preFight_RELOCATE");	
	local DummyDen=System:GetEntityByName("Den_preFight_RELOCATE");
	local AIDooch=System:GetEntityByName("Dooch");
	local AILock=System:GetEntityByName("Lock");
	local AIDen=System:GetEntityByName("Den");
	if((DummyDooch)and(AIDooch))then
...
function Mission:Event_MoveToFight()	
	local DummyDooch=System:GetEntityByName("Dooch_Fight_RELOCATE");	
	local DummyLock=System:GetEntityByName("Lock_Fight_RELOCATE");	
	local DummyDen=System:GetEntityByName("Den_Fight_RELOCATE");
	local AIDooch=System:GetEntityByName("Dooch");
	local AILock=System:GetEntityByName("Lock");
	local AIDen=System:GetEntityByName("Den");
	if((DummyDooch)and(AIDooch))then
