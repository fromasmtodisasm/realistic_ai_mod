...
function Mission:OnInit()
	...
	Language:LoadStringTable("specops06.xml");
	Hud:FlashObjectives({},"");
...
function Mission:Event_M_Goto_Forts()
	Hud:SetRadarObjective("Radar_Forts");
	Hud:AddMessage(Localize("NewObjective"),9);
	Hud:PushObjective({},Localize("Reserch_Stat"));
	Hud:PushObjective({},Localize("Two_Forts"));
	_localplayer.cnt.health=_localplayer.cnt.max_health;
	_localplayer.cnt.armor=_localplayer.cnt.max_armor;		 
end


function Mission:Event_M_Return_Station()
	Hud:SetRadarObjective("Radar_Station");
	Hud:AddMessage(Localize("NewObjective"),9);
	Hud:CompleteObjective(Localize("Two_Forts"));
	Hud:PushObjective({},Localize("Station6"));
end

function Mission:Event_M_Goto_Base()
	Hud:SetRadarObjective("Radar_Base");
	Hud:AddMessage(Localize("NewObjective"),9);
	Hud:CompleteObjective(Localize("Station6"));
	Hud:PushObjective({},Localize("Osn_Base"));
end

function Mission:Event_M_Intro_spec()
	Movie:PlaySequence('Intro_heli');
end

function Mission:Event_M_Open_Gate()
	Movie:PlaySequence('Station_gate_open');
end
...
function Mission:Event_MoveToPad()	
	local DummyDooch=System:GetEntityByName("Dooch_Pad_RELOCATE");	
	local DummyLock=System:GetEntityByName("Lock_Pad_RELOCATE");	
	local DummyDen=System:GetEntityByName("Den_Pad_RELOCATE");
...
function Mission:Event_MoveToFort()	
...
function Mission:Event_MoveToStat()	
	local DummyDooch=System:GetEntityByName("Dooch_Stat_RELOCATE");	
	local DummyLock=System:GetEntityByName("Lock_Stat_RELOCATE");	
	local DummyDen=System:GetEntityByName("Den_Stat_RELOCATE");
	local AIDooch=System:GetEntityByName("Dooch");
	local AILock=System:GetEntityByName("Lock");
	local AIDen=System:GetEntityByName("Den");
	if((DummyDooch)and(AIDooch))then
