...
function Mission:OnInit()
	...
	Language:LoadStringTable("specops03.xml");
	Hud:FlashObjectives({},"");
...
function Mission:Event_M_Fort_Engage()
	Hud:SetRadarObjective("Radar_Fort");
	Hud:AddMessage(Localize("NewObjective"),9);
	Hud:PushObjective({},Localize("Bunker_Comm"));
	Hud:PushObjective({},Localize("Engage_All"));
	_localplayer.cnt.health=_localplayer.cnt.max_health;
	_localplayer.cnt.armor=_localplayer.cnt.max_armor;		 
end


function Mission:Event_M_Bunker_Explosive()
	Hud:SetRadarObjective("Radar_Bunker");
	Hud:AddMessage(Localize("NewObjective"),9);
	Hud:CompleteObjective(Localize("Engage_All"));
	Hud:PushObjective({},Localize("Find_Down"));
end

function Mission:Event_M_Center_Explosive()
	Hud:SetRadarObjective("Radar_Center");
	Hud:AddMessage(Localize("NewObjective"),9);
	Hud:CompleteObjective(Localize("Find_Down"));
	Hud:PushObjective({},Localize("Badaboom"));
end

function Mission:Event_M_Goto_Village()
	Hud:SetRadarObjective("Radar_Village");
	Hud:AddMessage(Localize("NewObjective"),9);
	Hud:CompleteObjective(Localize("Badaboom"));
	Hud:PushObjective({},Localize("Go_To_Vilage3"));
end

function Mission:Event_M_Need_Boat()
	Hud:SetRadarObjective("Radar_Boat");
	Hud:AddMessage(Localize("NewObjective"),9);
	Hud:CompleteObjective(Localize("Go_To_Vilage3"));
	Hud:PushObjective({},Localize("Find_Boat3"));
end

function Mission:Event_M_Backto_Village()
	Hud:SetRadarObjective("Radar_Village");
	Hud:AddMessage(Localize("NewObjective"),9);
	Hud:CompleteObjective(Localize("Find_Boat3"));
	Hud:PushObjective({},Localize("Return_Vilage3"));
end
...
function Mission:Event_MoveToDam()	
	local DummyDooch=System:GetEntityByName("Dooch_Dam_RELOCATE");	
	local DummyLock=System:GetEntityByName("Lock_Dam_RELOCATE");	
	local DummyDen=System:GetEntityByName("Den_Dam_RELOCATE");
...
function Mission:Event_MoveToRvl()	
	local DummyDooch=System:GetEntityByName("Dooch_Rvl_RELOCATE");	
	local DummyLock=System:GetEntityByName("Lock_Rvl_RELOCATE");	
	local DummyDen=System:GetEntityByName("Den_Rvl_RELOCATE");
	local AIDooch=System:GetEntityByName("Dooch");
	local AILock=System:GetEntityByName("Lock");
	local AIDen=System:GetEntityByName("Den");
	if((DummyDooch)and(AIDooch))then
