...
function GameRules:OnCaptureStep_Touchable(checkpointno)
	...
	if next and next==checkpointno then
...
--------------------------------
	...
	OnTimer=function(self)
		if (self.switchTeamsCountdown and self.switchTeamsCountdown>0) then
			...
			if (self.switchTeamsCountdown==0) then
				...
				if toNumberOrZero(getglobal("gr_rm_needed_kills"))==0 then
					Server:BroadcastText("@GameOverSwitchTeamsIn "..self.switchTeamsCountdown,.9)
				end
...
function GameRules:ResetAtCurrentCheckpoint()
	...
	for i,slot in slots do
		local ent = System:GetEntity(slot:GetPlayerId())
	  	...
	  	if ent and ent.type=="Player" and Game:GetEntityTeam(ent.id)~="spectators" and ent.cnt.health >0 then
