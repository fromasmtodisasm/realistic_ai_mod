-- m_pScriptSystem->ExecuteFile("scripts/main.lua") В ините Game.cpp.
...
--	HeatVision.OnInit(HeatVision)
	...
	-- Script:ReloadScript("Scripts/Default/Entities/Player/Player.lua")
...
function ApplyPhysicsQuality(qual)
  	if (qual >= 2) then
	    ...
	    setglobal("p_max_contact_gap",.01)
  	elseif (qual==1) then
	    ...
	    setglobal("p_max_contact_gap",.015)
	end
    System:Log("Physics quality settings set to : "..qual)
end

function ApplyAutomaticSettings(qual)
  	if (qual==0) then
    	setglobal("ai_update_interval",.2)
	    setglobal("w_underwaterbubbles",0)
	    setglobal("ai_max_vis_rays_per_frame",50)
	    System:Log("Automatic settings set to low spec...")
	elseif (qual==1) then
    	setglobal("ai_update_interval",.1)
	    setglobal("w_underwaterbubbles",1)
	    setglobal("ai_max_vis_rays_per_frame",100)
	    System:Log("Automatic settings set to medium spec...")
	elseif (qual==2) then
    	setglobal("ai_update_interval",.1)
	    setglobal("w_underwaterbubbles",1)
	    setglobal("ai_max_vis_rays_per_frame",150)
	    System:Log("Automatic settings set to high spec...")
  	else
    	setglobal("ai_update_interval",.1)
	    setglobal("w_underwaterbubbles",1)
	    setglobal("ai_max_vis_rays_per_frame",300)
	    System:Log("Automatic settings set to very high spec...")
	end
end
