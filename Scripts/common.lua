...
----------
-- Изменено:
-- GetParticleCollisionStatus

-- Перезагружается через main.lua
...
-- function Localize(Token) -- Почти весь текст оранжевый.
	-- if (strsub(Token,1,1)~="@") then
		-- if Token then
			-- return "$8".."@"..Token
		-- end
...
function ConvertToRadAngles(dest,src)
	local x=rad(src.z+180)
	local y=rad((-src.x)+90)
	local z=rad(src.y)
...
function ConvertVectorToCameraAngles(dest,src)
		...
		if (temp.y==0 and temp.x==0) then
			...
			if (temp.x~=0) then
				...
				fYaw=atan2((temp.y),(temp.x))*(180/PI)
			...
			fPitch=(atan2(temp.z,fForward)*180/PI)
			if (fPitch<0) then
				fPitch=fPitch+360
			end
...
function ResetEnemies()
	...
	for i,entity in entities do
		if entity.Behaviour then
			System:Log("$6Resetting "..entity.GetName().."...")
			entity:OnReset()
--			if (entity.cnt) then
...
function GetPlayerWeaponInfo(player)
	...
	if (weaponid and weaponstate) then
...
function IsMaterialUnpierceble(material)
	if (material==nil) then
		System:Log("IsMaterialUnpierceble material is nil")
		return 1
	end
	if (material.gameplay_physic==nil) then
		System:Log("IsMaterialUnpierceble material.gameplay_physic is nil")
		return 1
	end
...
function ExecuteMaterial(pos,normal,material,bSound,target_id,ipart,PlaySoundObj)
	...
	if (decal) then
		...
		if (decal.random_scale) then
			scale=scale+((scale*.01)*random(0,decal.random_scale))
		end
		if (decal.random_rotation) then
...
function ExecuteMaterial2(hit,mat_field)
	...
	if not hit.target_material then	return end
	if not hit.target_material[mat_field] then
	--	System.LogToConsole("material is nil")
		do return end
	end
	...
	--SOUNDS------------------
	local MaterialSound
	if sounds and hit.play_mat_sound then
		local nsounds=getn(sounds)
		if (nsounds > 0) then
			...
			if not sound then
				...
				sound=Sound:Load3DSound(sounddesc[1],sounddesc[2],sounddesc[3],sounddesc[4],sounddesc[5])
				-- local Flags = bor(SOUND_OCCLUSION,SOUND_UNSCALABLE,SOUND_NO_SW_ATTENUATION)
				-- local Flags = 0
				-- sound=Sound:Load3DSound(sounddesc[1],Flags,sounddesc[3],sounddesc[4],sounddesc[5])
				-- add it to the table
				loadedsounds[slot]=sound
			--else
			--	local sounddesc=sounds[slot]
			...
			if AIInfo then
				-- AIInfo.fRadius = (sounds[slot][4]+sounds[slot][5])/2
				-- System:Log("DescCopy: "..sounds[slot][4].." + "..sounds[slot][5].." = "..AIInfo.fRadius)
...
--System:Log("decal >>> ID "..hit.target_id)
			...
			if (hit.ipart) then
				-- System:Log("1! "..hit.ipart)
				-- Hud:AddMessage("1! "..hit.ipart)
...
function GetParticleCollisionStatus(ent)
	...
	if (stat) then
		...
		if stat.target_material and stat.target_material.type~="obstruct2" then --obstruct -- stat.target_material - выдавало nil
			return stat
		end
...
function PointInsideBBox(point,ent,bias)
	...
	local normalizelen = 1/deltalen

	if  (deltalen <= 0) then normalizelen = .0001  end
...
-- function Logit(text)
	-- local file=openfile("mylog.txt", "a")
	-- write(file, text.."\n")
	-- closefile(file)
 -- end
