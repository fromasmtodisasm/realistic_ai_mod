...
function GameRules:OnAfterSpawnEntity(server_slot)
	...
	if newent.type=="Player" then
		...
		if (gr_InvulnerabilityTimer and toNumberOrZero(gr_InvulnerabilityTimer)>0) then
...
--  System:Log("Lib GameRules:OnDamage")
  ...
  if (theTarget and theTarget.type=="Player" and theTarget.invulnerabilityTimer and  theTarget.invulnerabilityTimer>0) then
...
--	        	    end
        	...
        	GameRules:OnMapChange()	-- init map entites
   	    	...
   	    	Server:BroadcastText("@CountdownNo "..(floor(gr_CountDown)-self.countnext),.95)
...
function GameRules:RespawnPlayer(server_slot)
	...
	if prevPlayerEntity and prevPlayerEntity.type=="Player" then
...
--	System:Log("OnSpectatorSwitchModeRequest curr host="..curhost)
	if (count(players)==0) then 
--		System:Log("OnSpectatorSwitchModeRequest no players")
		return 
	end 
			...
			if self.respawnList then
				for server_slot,respawn_info in self.respawnList do
					...
					if toNumberOrZero(getglobal("gr_keep_score"))==1 then
						if prevPlayerEntity and prevPlayerEntity.type=="spectator" then
							oldscore=prevPlayerEntity.cnt.score 
							olddeaths=prevPlayerEntity.cnt.deaths 
						end
					end
...
--			printf("SENT RTR! " .. GameRules.respawnCycleTimer)
	...
	for i,slot in slots do
		local player = GetSlotPlayer(slot)
		if (player and player.invulnerabilityTimer and player.ApplyTeamColor) then
