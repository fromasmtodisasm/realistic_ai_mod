...
function Hud:OnUpdate()
	...
	-- changed
	local bLocalIsAPlayer = player and (player.entity_type~="spectator") -- Интересный приёмчик.
	--place to check
	------------------------------------------
...
--					System:Log("Line 102 teamhud,should e setting name")
	...
	---------------------------------------------
	local fadeduration = .75  -- 2 second to fade
	local stayduration = 1.75  -- stay two second on screen with full alpha
	local translateduration = .65  -- 1.5 seconds to translate to the left

	local startwidth = 128
	local startheight = 128
	local startx = 400 - startwidth * .5  -- centered
	local starty = 300 - startheight * .5  -- centered
	...
	if (cl_display_hud~="0" and ClientStuff and not self.bHide) then
		...
		if (iGameState==CGS_INPROGRESS) then
			...
			if ((self.iRespawnCycleStart) and (self.iRespawnCycleStart)) then
