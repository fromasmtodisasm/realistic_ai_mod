...
function HeatVision:OnActivate()
	...
	CurrAmbientColor[1]=CurrAmbientColor[1]+1
	CurrAmbientColor[2]=CurrAmbientColor[2]+1
	CurrAmbientColor[3]=CurrAmbientColor[3]+1
...
function HeatVision:OnUpdate()
	...
	if (MyPlayer and MyPlayer.type=="Player" and not ClientStuff.vlayers:IsActive("Binoculars") and self.EnergyDecreaseRate and
	-- not MyPlayer.UnlimitedNighVision and (not MyPlayer.IsAiPlayer or (MyPlayer.IsAiPlayer and System:IsDevModeEnable()==1))) then
	not MyPlayer.UnlimitedNighVision and not MyPlayer.IsAiPlayer) then
