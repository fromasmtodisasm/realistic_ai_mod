...
function Binoculars:DrawOverlay()
	...
	local StaticFactor=(_time-Binoculars.LastChangeTime)/.3 
	if (StaticFactor<1) then
--		System:DrawRectShader("NoiseMask",0,0,800,600,.5,.5,.5,1-StaticFactor)
		local v=StaticFactor*2 
		System:DrawImageColorCoords(Binoculars.TID_Transition,0,0,800,600,4,1,1,1,1-StaticFactor*StaticFactor,0,.9+v,1,v)
	end
	local myPlayer=_localplayer 
	local u=(-myPlayer:GetAngles().z+217)/360+.27 
	System:DrawImageColorCoords(Binoculars.TID_Compass,401,185,60,16,4,1,1,1,1,u-.04,1,u+.04,0)
...
--	System:DrawRefractMask(Binoculars.TID_RefrMask,0,0,800,600)
	...
	--local EqLength=Sound:GetDirectionalAttenuationMaxScale()*.9+random(0,10)*.01 	-- lets enter some random value so the equalizer looks like it would really equalize something :p
	...
	--Game:WriteHudStringFixed(50,287,format("%02dX",(Binoculars.Zoom[ZoomView.CurrZoomStep].Factor)), 0,1,.9,.5,20,20,.6)
	Game:WriteHudString(50,292,format("%02dX",(Binoculars.Zoom[ZoomView.CurrZoomStep].Factor)), 0,1,.9,.5,15,15)
	...
	if (int_pt) then
		...
		--Game:WriteHudStringFixed(397,232,s,0,1,.9,.5,20,20,.6)
		Game:WriteHudString(400,232,s,0,1,.9,.5,15,15)
		--System:Log("pos="..int_pt.x..","..int_pt.y..","..int_pt.z.."|"..int_pt.angles.x..","..int_pt.angles.y..","..int_pt.angles.z)
	else
		Game:WriteHudString(400,232,"----.--m",0,1,.9,.5,15,15)
		--Game:WriteHudStringFixed(397,232,"----.--m",0,1,.9,.5,20,20,.6)
	end
end
