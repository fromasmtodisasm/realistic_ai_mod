ZoomView={
	IsActive = 0,
	--Zoomable = 1,
	-- NoZoom = 120 * 3.1415962 / 180, -- 90 * 3.1415962 / 180,
	...
	MaxSway = .4,
	-- SwayInc = 1,
	StanceSwayModifier = 1,
	AddZoom = 0,
	NumActive = 0,
	blend = 0,
	ZoomTime = .2,			--IronSights
}
--------------------------------------------------------
...
function ZoomView:Reset()
	...
	if IronSights and IronSights>="1" then
		_localplayer.cnt:SetSwayFreq(.6)		--IronSights
	end
end
--------------------------------------------------------
function ZoomView:Activate(_keymap,_sway,_fixedfactor,_fade)
	...
	if (ZoomView.NumActive==1) then
		...
		local IronSights = getglobal("g_enableironsights")
		if IronSights and IronSights=="0" then
			-- Input:SetMouseSensitivityScale(.3 * self.OldSensitivityScale)
			local ZoomStep=self.ZoomSteps[self.CurrZoomStep]
			if ZoomStep<1.8 then ZoomStep=1.8  end
			Input:SetMouseSensitivityScale(self.OldSensitivityScale/ZoomStep)			--IronSights
		end
		...
		--	e_detail_texture_min_fov = .01
		--else
		--	e_detail_texture_min_fov = 100
		--end
	end
...
function ZoomView:OnUpdate()
	...
	if (self.fade) then
		...
		-- end
		-- factor=delta/.5
		if (delta >= self.ZoomTime) then		--IronSights
			self.fade = nil
			delta = self.ZoomTime 				--IronSights
		end
		factor=delta/self.ZoomTime 				--IronSights
		self.currfov=self.NoZoom-(diff*factor)
		ZoomView.blend = factor
		--System:Log("ZoomView "..tostring(ZoomView.blend))
		if (self.fade==2) then									--IronSights
			self.currfov=(self.NoZoom-diff*floor(factor))
		end
...
function ZoomView:Deactivate()
	...
	local stats = player.cnt
	local IronSights = getglobal("g_enableironsights")
	if (IronSights and IronSights=="0") and stats.SetSwayAmp then						--IronSights
		stats:SetSwayAmp(0)
	end
	-- stats:SetSwayAmp(0)
	stats.zoom = 0
