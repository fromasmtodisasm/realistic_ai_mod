...
function DefaultZoomHUD:DrawHUD(ZoomStep)
    ...
    local fAdjustTexW=1/512 
    local fAdjustTexH=1/1024 
	...
	Game:WriteHudStringFixed(posZoomX,posZoomY,s,1,0,0,1,10,10,1)
	-- Draw distance
	...
	if (int_pt) then
		local s=format("%07.2fm",int_pt.len*1.5)
											  		  -- .5,20,20,.6
		Game:WriteHudStringFixed(posDistX,posDistY,s,1,0,0,1,10,10,1)
	else
		Game:WriteHudStringFixed(posDistX,posDistY,"----.--m",1,0,0,1,10,10,1)
	end
