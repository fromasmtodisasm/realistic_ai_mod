...
function ClientStuff:OnSave(stm)
	...
	stm:WriteBool(ClientStuff.GamePaused)
end
...
function ClientStuff:OnLoad(stm)
	...
	ClientStuff.GamePaused = stm:ReadBool()
end
...
-- function ClientStuff:RestoreAIranges() -- Да уж, действительно удваивается... Исправил в dll. Там после загрузки MakeAlerted вызывался, который удваивает текущую дальность видимости если AI_GunOut был равен еденице.
...
function ClientStuff:OnPauseGame()
	...
	self.temp_layers.Binoculars=self.vlayers:IsActive("Binoculars") -- Оставил потому, что при возвращении в игру скорость движения мыши установлена как без прицеливания.
	self.temp_layers.WeaponScope=self.vlayers:IsActive("WeaponScope")
	-- self.temp_layers.SmokeBlur=self.vlayers:IsActive("SmokeBlur")
	-- self.vlayers:DeactivateAll()
	...
	-- if _localplayer and _localplayer.cnt and _localplayer.bFired_Sht then
		-- _localplayer.bFired_Sht = 0
	-- end

	-- Убрав остановку огня здесь и сделав проверку в самом OnFire у сущности, повысил скорость установки игры на паузу! Особенно заметно на больших картах.
	ClientStuff.GamePaused = 1
	if _localplayer and not BasicPlayer.IsAlive(_localplayer) then -- Если игрок уже умер и игра была поставлена на паузу, то поставить громкость на максимум, иначе музыка не играет.
		-- System:Log("SOUNDSCALE_DEAFNESS,1")
		Sound:SetGroupScale(SOUNDSCALE_DEAFNESS,1)
	end
end

function ClientStuff:OnPauseGameStopFiring() -- А это для C++ при включении кат сцен.
	local Entities
	if GameRules.AllEntities then
		Entities = GameRules.AllEntities
	end
	if not Entities then
		System:Log(_localplayer:GetName()..": $1ClientStuff/OnPauseGame/ Use System:GetEntities()")
		Entities=System:GetEntities() -- На всякий случай, а то кое-какой, но долгий лаг при установке на паузу на очень больших картах вызывает.
	end
	for i,entity in Entities do
...
function ClientStuff:OnResumeGame() -- Теперь при окончании кат сцен точно вызывается.
	-- System:Log("RESUME")
	for i,val in self.temp_layers do
		self.vlayers:ActivateLayer(i) -- Может, бинокль при возвращени активируется только "поверхностно"?
	end
	ClientStuff.GamePaused = nil
end
...
function ClientStuff:OnTextMessage(Command,SenderName,Text)
	-- if SenderName then
		...
		-- local entity=System:GetEntityByName(SenderName)
		-- if entity then BasicPlayer:OnTextMessage(entity,Command,SenderName,Text) end
	-- else
		-- Hud:AddMessage(Text)
	-- end

	if not Game:IsMultiplayer() and SenderName then
		local SenderEntity=System:GetEntityByName(SenderName)
		if SenderEntity and SenderEntity==_localplayer then
			local Entities
			if GameRules.AllEntities then
				Entities = GameRules.AllEntities
			end
			if Entities then
				for i,entity in Entities do
					if entity.IsAiPlayer and BasicPlayer.IsAlive(entity) then
						BasicPlayer:OnTextMessage(entity,Command,SenderName,Text)
					end
				end
			end
		end
