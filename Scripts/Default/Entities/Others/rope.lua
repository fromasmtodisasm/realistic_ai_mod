...
--System:Log("\001 DROP second -------------  "..self.dropEntity1.cnt.health.."  "..self.dropEntity1:GetName().."  "..self.dropEntity1.id)

--System:Log("\001 DROP second -------------  "..self.state)
...
--			else
--System:Log("\001 DEAD -------------  "..self.dropEntity1:GetName())
			end

--System:Log("\001 signal -------------  "..self.dropEntity1:GetName().."  "..self.dropEntity1.id)
--			if (self.theHely) then
				--System:Log("\001 signal -------------  "..self.theHely.id)
...
System:Log(" signalling  >>  "..self.dropEntity1.cnt.health)
			...
			AI:Signal(0,1,"exited_vehicle2",self.dropEntity1.id)
			self.dropEntity1 = nil 
			if (self.dropEntity2) then
				AI:Signal(SIGNALID_READIBILITY,2,"AI_AGGRESSIVE_MACHO",self.dropEntity2.id)
				self:Descend()
			else
				self:Retrieve()
			end
		elseif (self.state==1) then
			AI:Signal(SIGNALID_READIBILITY,2,"AI_AGGRESSIVE_MACHO",self.dropEntity1.id)					-- start bone descend animation
			self:Descend()
...
--System:Log("second -------------  "..self.state)
		...
		if (self.state==5 or self.dropEntity2.cnt.health==0) then	-- already down or dead
			...
			AI:Signal(0,1,"exited_vehicle2",self.dropEntity2.id)
...
--System:Log("\001 event Descended")
			self.state = 5 
