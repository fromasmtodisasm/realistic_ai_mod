...
--Hud.label = "inside ProximityDamage entity" 
	...
	if (self.Properties.nShakeType==0) then
		local ShakeAmount = self.Properties.fDamageRate * 2.2 		--was .5
		...
		collider.cnt:ShakeCameraL(.1,11.73,1.73)
	end
...
function ProximityDamage:CanBeDamaged(collider)
	...
	if (collider.cnt.health <= 0) then return 0 end	
	if (collider.Properties.bInvulnerable and collider.Properties.bInvulnerable==1) then return 0 end
	if ((self.Properties.bSkipPlayer==1) and (collider==_localplayer or collider.IsAiPlayer)) then return 0 end
	if ((self.Properties.bSkipAI==1) and (collider.ai) and (not collider.IsAiPlayer)) then return 0 end
