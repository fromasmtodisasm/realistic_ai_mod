...
--	if (Game:GetWaterHeight()>self.emtrPos.z) then return end
	...
	if (self.piece_idx < 2) then
		...
		if (self.bParticleEmitter==nil) then
			...
			self:CreateParticleEmitterEffect(0,self.DeadEffect,self.part_update_freq/1000,self.emtrPos,g_Vectors.v001,1)
		end
	else
...
function Piece:DoSplash(pos)
	...
	local volume = min(sqrt(deltax*deltay*deltaz),100)*.015 
