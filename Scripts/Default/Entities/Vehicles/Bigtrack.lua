...
--			-- don't change action map to vehicle when entering
	...
	--
	DamageParams = {
		-- fDmgScaleAIBullet = .02,
		-- fDmgScaleAIExplosion = .115, --patch2:3 missiles to be destroyed --before was .03,
		fDmgScaleBullet = .01,
		fDmgScaleExplosion = .125, --patch2:3 missiles to be destroyed --before was .03,

		-- dmgBulletMP = .2, --if this value exist in multiplayer will be used this damage for every bullet ,
...
--	szNormalModel="Objects/Vehicles/HEMTT/hemtt_driveable.cgf",
	...
	CarDef = {
		...
		max_braking_friction = .5,
		handbraking_value = 9, -- meter/s / s

		max_braking_friction_nodriver = 1, --this friction is applied when there is no driver inside the car
	  ...
	  cam_snap_vel = .01,
		...
		max_steer_v0 = 25, --max steer angle
		--max_steer_kv = 0,
...
--		hull0 = {mass=1859,flags=0,zoffset=.41	}, -- default mass 1859
		hull1 = {mass=50000,flags=0,zoffset=-.5,yoffset=0	}, -- default mass 2359
		hull2 = {mass=0,flags=1	},
		hull3 = {mass=0,flags=1	},

		wheel1 = {driving=1,axle=0,len_max=.3,stiffness=0,damping=-.7, surface_id = 69, min_friction=1, max_friction=1.5},
		wheel2 = {driving=1,axle=0,len_max=.3,stiffness=0,damping=-.7, surface_id = 69, min_friction=1, max_friction=1.5},
		wheel3 = {driving=1,axle=1,len_max=.3,stiffness=0,damping=-.7, surface_id = 69, min_friction=1, max_friction=1.5},
		wheel4 = {driving=1,axle=1,len_max=.3,stiffness=0,damping=-.7, surface_id = 69, min_friction=1, max_friction=1.5},
		wheel5 = {driving=1,axle=2,len_max=.3,stiffness=0,damping=-.7, surface_id = 69, min_friction=1, max_friction=1.5},
		wheel6 = {driving=1,axle=2,len_max=.3,stiffness=0,damping=-.7, surface_id = 69, min_friction=1, max_friction=1.5},
		wheel7 = {driving=1,axle=3,len_max=.3,stiffness=0,damping=-.7, surface_id = 69, min_friction=1, max_friction=1.5},
		wheel8 = {driving=1,axle=3,len_max=.3,stiffness=0,damping=-.7, surface_id = 69, min_friction=1, max_friction=1.5},
...
--	counting = nil,
	...
	Properties = {
		...
		ExplosionParams = {
			...
			fRadius = 14, -- default 17
			fImpulsivePressure = 200, -- default 200 Умножается на 2.
		},
...
--	} 
	...
	self.idleengine_ratios = {.9,.9,1.15,.9} 
...
function Bigtrack:DropDriver()
	VC.DropDriver(self)
end

function Bigtrack:DropPassengers()
