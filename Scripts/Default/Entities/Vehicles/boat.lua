...
--	type = "Vehicle",
	...
	DamageParams = {
		-- fDmgScaleAIBullet = .1,
		-- fDmgScaleAIExplosion = .335, --patch2:vehicles must explode with 1 missile --before was .4,
		fDmgScaleBullet = .017, -- .05 -- Не забывай про self.fEngineHealth в OnReset.
		fDmgScaleExplosion = .15, --patch2:vehicles must explode with 1 missile --before was 1,

		-- dmgBulletMP = 1, --if this value exist in multiplayer will be used this damage for every bullet ,
...
--		Dampw		= 300000,	--waves dump
		...
		TiltSpdA	= .05,	--tilt momentum when speeding up (acceleration thrhld)
		TiltSpdMinV	= 10,	--tilt momentum when speeding up (min speed to tilt when not accelerating)
		TiltSpdMinVTilt	= .17,	--tilt momentum when speeding up (how much to tilt when not accelerating)
		fMass 		= 15000,
		Flying		= 0,
...
--	fMass = 2500,
	...
	particles_updatefreq = .24, --initial frequency of updating particles
...
function Boat:UpdateClientAlive(dt)
	...
	if (self.gunnerT and self.gunnerT.entity and self.gunnerT.state==2) then -- С исправлением на загребущие ручки...
...
function Boat:DropDriver()
	VC.DropDriver(self)
end

function Boat:DropGunner()
	VC.DropGunner(self)
end
...
--
--
function Boat:DoEnter(puppet) -- Вернуть!
...
function Boat:AddGunner(puppet)
	if self.gunnerT.entity then	-- already have a driver
		do return 0 end
	end
