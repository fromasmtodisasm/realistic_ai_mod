...
function SmartProximityTrigger:Event_Enter(sender)
	...
	self.Entered = 1 
	if (sender) then
		System:Log("Player "..sender:GetName().." Enter SmartProximityTrigger "..self:GetName())
	end
...
function SmartProximityTrigger:Event_Leave(sender)
	...
	System:Log("Player "..sender:GetName().." Leave SmartProximityTrigger "..self:GetName())
...
SmartProximityTrigger.Empty = 
{
	...
	OnContact = function(self,player)
		...
		if (self.Properties.bOnlyMyPlayer~=0 and player~=_localplayer and not entity.IsAiPlayer) then
			return
		end
