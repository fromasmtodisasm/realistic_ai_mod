	...
	if (id) then
		...
		if (slot) then
			if Game:IsMultiplayer() then
				slot:SendText(msg,.9) -- Что-то эта штука сломалась в одиночке. И это почему-то не зависит от запущенной копии фарика. Сброс настроек редактора тоже не помогает. Значит, дело где-то в винде.
			else
				Hud:AddMessage(msg,.9)
			end
		end
	end
end
