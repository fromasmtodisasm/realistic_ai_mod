...
--	self.Entered = 1 
	...
	if (sender and sender.type=="Vehicle") then
		...
		System:Log("Boat entered with speed2="..spd2.."Angle="..angleImpact)
		...
		if (spd2 >= self.MinSpeed2 and angleImpact <= self.Properties.MaxAngleOfImpact) then
			...
			if (self.Properties.ScriptCommand and self.Properties.ScriptCommand~="") then
				--System:Log("Executing: "..self.Properties.ScriptCommand)
				dostring(self.Properties.ScriptCommand)
			end
		
		
			if (self.Properties.PlaySequence~="") then
				System:Log("Playing sequence: ".. self.Properties.PlaySequence)
	...
	if (sender) then
		System:Log("Player "..sender:GetName().." Enter BoatTrampolineTrigger "..self:GetName())
	end
end
...
function BoatTrampolineTrigger:Event_Leave(sender)
	...
	self.Entered = 0 
	if (sender) then
		System:Log("Player "..sender:GetName().." Leave BoatTrampolineTrigger "..self:GetName())
	end
...
function BoatTrampolineTrigger:OnTimeF()
	...
	if (self.boat) then
		...
		System:Log("Applying impulse ="..self.MaxImpulse * coeff)
