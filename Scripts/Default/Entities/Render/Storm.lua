Storm = {
	...
	Properties = {	
		fRainAmount = .5, -- amount of rain,range [0..1]
	...
	LightningParticles = {
		...
		start_color = {1,1,1},
		end_color =  {1,1,1},
		rotation={x=0,y=0,z=0},-- default z=1.75
		speed = 0, -- default 0
		count = 1, -- default 2
		size = 512,size_speed=0, -- default size = .025 -- default size_speed = .5
		gravity={x=0,y=0,z=0},-- default z=.2
		lifetime=.75, -- default .75
		frames=9,
		blend_type = 2,
...
function Storm:SetThunder()
	...
	if (self.nCurrentPos==1) or (self.nCurrentPos==4) then
		...
		Particle:CreateParticle(explPos,{x = 0,y = 0,z = 1},self.LightningParticles)
	...
	if (self.nCurrentPos==10) then	
		...
		self.fCurrentRain=random(0,16*self.Properties.fRainAmount)/16 
