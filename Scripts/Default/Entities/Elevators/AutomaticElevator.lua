...
function AutomaticElevator:IsMovingClient(dt)
	...
	if (self.Properties.WarnLight.bHasWarnLight>0) then
		...
		if (Pos) then
			...
			--System:Log("LightPos("..Pos.x..","..Pos.y..","..Pos.z..")")
			--System:Log("LightAng("..self.Properties.WarnLight.LightAngles.x..","..self.Properties.WarnLight.LightAngles.y..","..self.Properties.WarnLight.LightAngles.z..")")
...
function AutomaticElevator:OnReset()
	...
	if (self.Properties.OpenDelay<=0) then
		self.Properties.OpenDelay=.001
	end
	if (self.Properties.RetriggerDelay<=0) then
		self.Properties.RetriggerDelay=.001
	end
...
--	System.Log("$4SETTING DURINGLOADING 1 !!!")
	...
	--System:Log("ProjId="..self.proj_tex_id)
...
function AutomaticElevator:OnEnterArea(Player,AreaId)
	Player.InElevatorArea = 1 -- Как тут id достать?
	BasicPlayer.FriendFollow(Player,1)
	if not self.PlayersTable then self.PlayersTable = {} end
	if not self.TheNumberOfRows then self.TheNumberOfRows=0 end
	for i,val in self.PlayersTable do -- i всегда начинается с еденицы.
	...
	for i=0,self.TheNumberOfRows do
		if i==self.TheNumberOfRows then
			self.PlayersTable[i+1]=Player
		end
	end
	-- Hud:AddMessage(Player:GetName()..": On enter elevator area, AreaId: "..AreaId)
	-- System:Log(Player:GetName()..": On enter elevator area, AreaId: "..AreaId)
end

function AutomaticElevator:OnLeaveArea(Player,AreaId)
	BasicPlayer.FriendFollow(Player)
	Player.InElevatorArea = nil
	if not Player.IsAiPlayer and Player.cnt then
		if Player.cnt.proning then
			Player:InsertSubpipe(0,"setup_crouch")
			Player:InsertSubpipe(0,"do_it_walking")
		elseif Player.cnt.crouching then
			Player:InsertSubpipe(0,"setup_stand")
			if random(1,2)==1 then
				Player:InsertSubpipe(0,"do_it_walking")
			else
				Player:InsertSubpipe(0,"do_it_running")
			end
		end
	end
	if self.PlayersTable then
		for i,entity in self.PlayersTable do
			if Player.id==entity.id then
				self.PlayersTable[i]=nil
				-- Hud:AddMessage(Player:GetName()..": entity = nil")
				-- System:Log(Player:GetName()..": entity = nil")
				break
			end
		end
	end
	-- Hud:AddMessage(Player:GetName()..": On leave elevator area, AreaId: "..AreaId)
	-- System:Log(Player:GetName()..": On leave elevator area, AreaId: "..AreaId)
end

function AutomaticElevator:OnMove()
	if self.PlayersTable then
		for i,entity in self.PlayersTable do
			if entity.ai and entity.InElevatorArea and BasicPlayer.IsAlive(entity) and AI:IsMoving(entity.id) then
				AI:EnablePuppetMovement(entity.id,0,.2)
				...
				-- Hud:AddMessage(entity:GetName()..": do not move in elevator!")
				-- System:Log(entity:GetName()..": do not move in elevator")
				break
			end
		end
	end
	-- Hud:AddMessage(self:GetName()..": On elevator move")
	-- System:Log(self:GetName()..": On elevator move")
end

-- function AutomaticElevator:OnProceedFadeArea(Player,AreaId,FadeCoeff) -- Нет такого здесь.
	-- Hud:AddMessage(Player:GetName()..": On elevator fade area, AreaId: "..AreaId..", FadeCoeff: "..FadeCoeff)
	-- System:Log(Player:GetName()..": On elevator fade area, AreaId: "..AreaId..", FadeCoeff: "..FadeCoeff)
-- end
------------------
------------------
-- SERVER
...
--				System.LogToConsole("InUpPos")
				System:Log("Opened State Timer Exprired")
				self:GotoState("Closing")
		...
		OnEnterArea=AutomaticElevator.OnEnterArea,
		OnLeaveArea=AutomaticElevator.OnLeaveArea,
		OnMove=AutomaticElevator.OnMove,
		-- OnProceedFadeArea=AutomaticElevator.OnProceedFadeArea,
	},
	------------------
	--OPENING
	------------------
