...
function Ladder:OnReset()
	...
	self.lastSound=nil
	if self.Properties.fHAngleLimit==-1 then
		self.Properties.fHAngleLimit=90
	end
end
...
function Ladder:OnEnterArea(player)
	if player.type~="Player" or player.theVehicle or player.MUTANT or player.ANIMAL then return end -- this player is in some vehicle - can't use ladder
...
--		player.cnt:OverrideMoveParams(self.climb_move_params)
		...
		if (lockPt) then
			-- System:Log(self:GetName()..": lockPt")
			local zDist = lockPt.z - player:GetPos().z
			if (zDist>-self.Properties.LockDist and zDist<self.Properties.LockDist) then
				-- System:Log(self:GetName()..": zDist: "..zDist..", self.Properties.LockDist: "..self.Properties.LockDist)
...
function Ladder:OnLeaveArea(player)
	if player.type~="Player" or player.MUTANT or player.ANIMAL then return end
	-- if (player.ai) then return end	-- we don't want AIs to use ladders anymore
	if player.ai then
...
--
--	--System:Log("speed="..player.cnt.vel)
--	if (player.cnt.vel>1) then
--		local Snd=self.ladder_sound[random(1,4)]
--		if (self.lastSound==nil) then
--			self.lastSound=Snd
