...
function AutomaticDoor:OnReset()
	...
	self:SetBBox({x=-(self.Properties.BBOX_Size.X*.5),y=-(self.Properties.BBOX_Size.Y*.5),z=-(self.Properties.BBOX_Size.Z*.5)},
	{x=(self.Properties.BBOX_Size.X*.5),y=(self.Properties.BBOX_Size.Y*.5),z=(self.Properties.BBOX_Size.Z*.5)})
	self:RegisterState("Inactive")
	self:RegisterState("Opened")
	self:RegisterState("Closed")

	if (self.Properties.iNeededKey~=-1) then
		--System:Log("Need the key!")
		self.bLocked=1
	else
		self.bLocked=0
	end
	...
	self.Timer=0
	-- AI:EnableNodesInSphere(self:GetPos(),self.Properties.BBOX_Size.X,1)
	self:UpdatePortal()
end
...
function AutomaticDoor:Event_Open(sender)
	...
	if sender and self.Properties.AISoundEvent.bEnabled==1 then
...
function AutomaticDoor:Event_Opened(sender)
	...
	if (self.Properties.AISoundEvent.bEnabled==1) and (bAutomatic==0) then
...
function AutomaticDoor:OnOpen(self,other,usepressed)
	...
	if (self.Properties.bAutomatic==1 or (self.Properties.bAutomatic==0 and (usepressed==1 or (other.ai and (other.MERC or other.IsAiPlayer or other.IsSpecOpsMan or other.MUTANT~="predator"))))) and not self.EndReached then
...
AutomaticDoor["Server"]={
	...
	------------------
	Inactive = {
		-- OnEnterArea=AutomaticDoor.OnEnterArea,
		-- OnLeaveArea=AutomaticDoor.OnLeaveArea,
		OnContact = function(self,other)
			AI:EnableNodesInSphere(self:GetPos(),self.Properties.BBOX_Size.X,0)
		end,
	},
	------------------
	--OPENED
...
--		end,
		...
		OnEvent = function(self,eid)
			if ((eid==ScriptEvent_OnTimer) and (self.Properties.bUseAnimatedModel==0)) then
...
AutomaticDoor["Client"]={
	...
	Opened = {
		...
		OnUpdate = function(self,dt)
			...
			--System:Log("Automatic door client update")
			...
			if ((self.numupdates==5) and (self.OpenSound) and (not Sound:IsPlaying(self.OpenSound))) then
				...
				--System:Log("Automatic Door Open("..self:GetPos().x..","..self:GetPos().y..","..self:GetPos().z..",")
	...
	Closed = {
		...
		OnBeginState = function(self)
			...
			--System:Log("Automatic Door Close("..self:GetPos().x..","..self:GetPos().y..","..self:GetPos().z..",")
			self:EnableUpdate(1)
