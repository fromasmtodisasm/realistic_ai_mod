Door = {
	Properties = {
		...
		AnimationSpeed=1,
		AnimationOpen="back",
		AnimationOpenBack="front",
		AnimationClose="",
		vector_Bounds={x=0,y=0,z=0},
  		bPlayerOnly = 0,
		object_AnimatedModel= "Objects/Buildings/M03/compound_area/coainstantshelterdoor.cga",
  		bAutomaticDoor = 0,
  		bEnabled = 1,
  		iNeededKey=-1,
		bUsePortal=1,
		...
		Sound={
			sound_Open = "Sounds/doors/wooddoorclose.wav",
  			sound_Close = "Sounds/doors/wooddooropen.wav",
			sound_Unlock = "Sounds/items/button1.wav",
  			InnerRadius = 1,
			OuterRadius = 25,
			nVolume = 255,
			},
...
function Door:OnPropertyChange()
	...
	--{x=-(self.Properties.vector_Bounds.x*.5),y=-(self.Properties.vector_Bounds.y*.5),z=-(self.Properties.vector_Bounds.z*.5)},
	--{x=(self.Properties.vector_Bounds.x*.5),y=(self.Properties.vector_Bounds.y*.5),z=(self.Properties.vector_Bounds.z*.5)})
...
function Door:CanOpenDoor(entity)
	if (self.Properties.bPlayerOnly==1 and entity~=_localplayer and not entity.IsAiPlayer) then
		return 0
	end

	if (entity.ai and entity.type~="Player" and not entity.IsAiPlayer) then
...
function Door:OnReset()
	...
	self.refcounter=0
	if self.max_box then
		-- AI:EnableNodesInSphere(self:GetPos(),self.max_box.y,1)
	end
end
...
function Door:OpenDoor(bPlayFront)
	...
	if (self:GetState()=="Closing" and self.Properties.AnimationClose=="") then
		...
		self:StartAnimation(0,sAnimName,0,.5,self.Properties.AnimationSpeed)
	end
end
...
--		System:Log("EVENT OPEN ("..self:GetName()..") ")
		...
		local bPlayFront = 1
		if (self.Properties.AnimationOpenBack~="" and self.WhoOpens) then
...
--		System:Log("**EVENT** Close")
		...
		if (self.Properties.AnimationClose~="") then
			...
			self:StartAnimation(0,self.Properties.AnimationClose,0,.5,self.Properties.AnimationSpeed)
		else
			self:SetAnimationSpeed(-self.Properties.AnimationSpeed)
		end
...
--		System:Log("**EVENT** Opened")
		...
		-- Hud:AddMessage(self:GetName()..": Opened SIGNAL_GENERATED")
...
--		System:Log("**EVENT** Closed")
		...
		-- Hud:AddMessage(self:GetName()..": Closed SIGNAL_GENERATED")
		self:GotoState("Closed")
	end
end
...
function Door:OnOpen(self,other,usepressed)
	...
	self.WhoOpens = other
	if ((self.Properties.bAutomaticDoor==1) or ((self.Properties.bAutomaticDoor==0) and (usepressed==1))) then --doors have to be opened by player
		if (self.bLocked~=0) then
		...
		-- if (self.Properties.AISoundEvent.bEnabled==1) then
			-- if (self.AI_SIGNAL_GENERATED==nil) then
				-- _localplayer.sspecies = other.Properties.species
				-- if (other.ai) then
					-- other:ChangeAIParameter(AIPARAM_SOUNDRANGE,0)
				-- end
				-- -- Hud:AddMessage(self:GetName()..": Opening SIGNAL_GENERATED")
				-- AI:SoundEvent(self.id,self:GetPos(),self.Properties.AISoundEvent.fRadius,0,1,_localplayer.id)
				-- Game:SoundEvent(self:GetPos(),self.Properties.AISoundEvent.fRadius,1,0)
				-- if (other.ai) then
					-- other:ChangeAIParameter(AIPARAM_SOUNDRANGE,other.PropertiesInstance.soundrange)
					-- -- Hud:AddMessage(_localplayer:GetName()..": _localplayer.sspecies: ".._localplayer.sspecies)
					-- -- _localplayer.sspecies = nil  -- Нельзя чтобы sspecies был равен nil,иначе свои у ботов начинают реагировать на открытие.
				-- end
		...
		if (self.Properties.AISoundEvent.bEnabled==1) then
			if (self.AI_SIGNAL_GENERATED==nil) then
				...
				-- end
				-- Hud:AddMessage(self:GetName()..": Opening SIGNAL_GENERATED")
				AI:SoundEvent(other.id,other:GetPos(),self.Properties.AISoundEvent.fRadius,0,1,other.id)
				-- Game:SoundEvent(self:GetPos(),self.Properties.AISoundEvent.fRadius,1,0)
				-- if (other.ai) then
					-- other:ChangeAIParameter(AIPARAM_SOUNDRANGE,other.PropertiesInstance.soundrange)
					-- -- Hud:AddMessage(_localplayer:GetName()..": _localplayer.sspecies: ".._localplayer.sspecies)
...
function Door:OnClose(self,other,usepresse) -- ИИ не может закрыть дверь сам((
	if (not other.cnt) then
		return 0
	end

	if (self:CanOpenDoor(other)==1) then
		-- if (self.Properties.AISoundEvent.bEnabled==1) and (self.Properties.bAutomaticDoor==0) then
			-- if (self.AI_SIGNAL_GENERATED==nil) then
				-- _localplayer.sspecies = other.Properties.species
				-- if (other.ai) then
					-- other:ChangeAIParameter(AIPARAM_SOUNDRANGE,0)
		...
		if (self.Properties.AISoundEvent.bEnabled==1) and (self.Properties.bAutomaticDoor==0) then
			if (self.AI_SIGNAL_GENERATED==nil) then
				...
				-- if (other.ai) then
					-- other:ChangeAIParameter(AIPARAM_SOUNDRANGE,0)
				-- end
				AI:SoundEvent(self.id,self:GetPos(),self.Properties.AISoundEvent.fRadius,0,1,other.id)
				-- Game:SoundEvent(self:GetPos(),self.Properties.AISoundEvent.fRadius,1,0)
				-- if (other.ai) then
					-- other:ChangeAIParameter(AIPARAM_SOUNDRANGE,other.PropertiesInstance.soundrange)
				-- end
...
function Door:OnUse(player)
	if (self.isclosed==1 and self.Properties.bAutomaticDoor==0 and (self.bEnabled==1)) then
		self:OnOpen(self,player,1)
		return 1
	elseif (self.isclosed==0 and self.Properties.bAutomaticDoor==0 and (self.bEnabled==1)) then
		self:OnClose(self,player,1)
		return 1
	end
	return 0
end
...
function Door:OnEnterArea(Player,AreaId)
	Player.AutomaticDoorArea = 1
	BasicPlayer.FriendFollow(Player,1)
	-- Hud:AddMessage(Player:GetName()..": On enter area, AreaId: "..AreaId)
end

function Door:OnLeaveArea(Player,AreaId)
	BasicPlayer.FriendFollow(Player)
	Player.InAutomaticDoorArea = nil
	-- Hud:AddMessage(Player:GetName()..": On leave area, AreaId: "..AreaId)
end
...
Door["Server"]={
	...
	Inactive = {
		OnContact = function(self,other)
			if self.max_box then
				AI:EnableNodesInSphere(self:GetPos(),self.max_box.y,0)
			end
		end,
	},
	------------------
...
--			System:Log("BEGIN OPENING "..self:GetName())
		...
		-- OnEnterArea=AutomaticDoor1Piece.OnEnterArea,
...
--			System:Log("BEGIN OPENED "..self:GetName())
			if (self.Properties.bAutomaticDoor==1) then
	...
	----------------------------------------------------------
		OnContact = function(self,other)
			--System:Log("SERVER:OnContact")

			if (self.Properties.bAutomaticDoor==0) and (not other.ai) then
				if (other==_localplayer) then
					Hud.label=Localize("CloseDoor")
					-- Hud:AddMessage(self:GetName()..": TEXT: OpenDoor")
				end
				do return end
			end

			-- self:CanOpenDoor(other)
...
--			System:Log("ON CONTACT")
			...
			if (((self.bLocked==0 or self:HasKeycard(other)) and self.Properties.bAutomaticDoor==0) and (not other.ai)) then -- ai can always open the door
