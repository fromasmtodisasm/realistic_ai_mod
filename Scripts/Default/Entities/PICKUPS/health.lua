...
--	printf("collider health is %d < %d >  tm< %d >",collider.cnt.health,self.Properties.Amount,self.Properties.RespawnTime)
	...
	if collider.cnt.health and collider.cnt.health > 0 then -- Чтобы трупы не подбирали. Проверить.
		collider.cnt.health = collider.cnt.health + floor(self.Properties.Amount*(collider.cnt.max_health/100))
		if (collider.cnt.health>collider.cnt.max_health) then
	...
	--end
	if Game:IsMultiplayer() then -- Этого не было.
...
--  	FastScaleVector(dirn,dirn,.15) FastScaleVector(dirs,dir,1.5)
...
--	end
	...
	if (hits and getn(hits)>0) then
		...
		dest.x = temp.x - direction.x * .15
		dest.y = temp.y - direction.y * .15
		dest.z = temp.z - direction.z * .15
	end
