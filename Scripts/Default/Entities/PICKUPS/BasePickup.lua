BasePickup={
	Properties = {
		...
		bPlayerOnly = 0, -- IsInteractionPossible
		bShowFloatingIcon = 0,
		bAwakePhysics = 0,
...
function BasePickup:RegisterWithAI()
	if 	self.ammo_type=="FlashbangGrenade" or self.ammo_type=="HandGrenade" or self.ammo_type=="SmokeGrenade" then
		self.Properties.Amount = 1
	end
	if self.pickuptype=="weapon" then
		pickup = AIOBJECT_WEAPON
		-- typepname = "AIOBJECT_WEAPON"
	elseif self.pickuptype=="ammo" then
		pickup = AIOBJECT_AMMO
		-- typepname = "AIOBJECT_AMMO"
	elseif self.pickuptype=="health" then -- Не определило.
		pickup = AIOBJECT_HEALTH
		-- typepname = "AIOBJECT_HEALTH"
	elseif self.pickuptype=="armor" then
		pickup = AIOBJECT_ARMOR
		-- typepname = "AIOBJECT_ARMOR"
	else
		pickup = AIOBJECT_OTHER
		-- typepname = "AIOBJECT_OTHER"
	end
	AI:RegisterWithAI(self.id,pickup)
	-- System:Log(self:GetName().." RegisterPickup: "..typepname) -- Забыл как значение переменной в виде текста получить.
	if self.Properties.KeyNumber then -- Правильная модель с соответствующим цветом карточки.
		if self.Properties.KeyNumber==4 then
			self.Properties.object_Model = "Objects/Pickups/misc/keycard_ylw.cgf"
		elseif self.Properties.KeyNumber==1 then
			self.Properties.object_Model = "Objects/Pickups/misc/keycard_red.cgf"
		elseif self.Properties.KeyNumber==2 then
			self.Properties.object_Model = "Objects/Pickups/misc/keycard_grn.cgf"
		elseif self.Properties.KeyNumber==3 then
			self.Properties.object_Model = "Objects/Pickups/misc/keycard_blue.cgf"
		end
		-- System:Log(self:GetName()..": KeyNumber: "..self.Properties.KeyNumber..", Model: "..self.Properties.object_Model)
	end
	if self.pickuptype=="armor" then -- Вся броня всегда даётся в количестве 100%.
		self.Properties.Amount = 100
	end
end
...
function BasePickup:Physicalize(spawner)
	if (self.physics and (self.Properties.RespawnTime==0 or self.Properties.AlwaysPhysicalize)) then
		if (not self.physicalized or spawner) then
...
function BasePickup:NotifyMessage(msg,target,lifetime)
	if (target==nil) then
		...
		if slot then
			-- if Game:IsMultiplayer() then -- Почему так? Об этом рассказано в PlaceableExplo.lua
				if lifetime then
...
function BasePickup:AttemptPick(collider,entering)
	if (collider.type=="Player") and (collider.cnt.health>0) then
		-- if (collider==_localplayer) and (not _localplayer.cnt.use_pressed) then
		-- http://www.farcry.ru/forum/viewtopic.php?t=1533&postdays=0&postorder=asc&start=160
			-- Hud.label = "[$1Pick up the item]"
			-- return
		-- end
		if (collider.theVehicle) then return end		-- no pickups when in vehicles
		if (GameRules:IsInteractionPossible(collider,self,1)==nil) then return end
		if collider.ANIMAL or (collider.MUTANT and collider.MUTANT~="fast" and collider.MUTANT~="stealth") then return nil end
...
function BasePickup:LoadGeometry()
	...
	elseif (self.rotate90) then
		self:SetObjectPos(0,{x=0,y=0,z=.1})
	end
end
------------------------------------------------------
-- game_Difficulty
...
BasePickup.Server={
	...
	Active={
		OnBeginState = function(self)
			--Begin = 1
			self:RegisterWithAI() -- Это не AI:RegisterWithAI, а self:RegisterWithAI! Не забывай!
			self.ammopicked = nil
		...
		OnUpdate = function(self)
			-- if not self.ForceExitSearchAmmo and Begin then self.ForceExitSearchAmmo = 0 Begin=nil end
			-- if self.ForceExitSearchAmmo then -- Эктив ловит и на этом прекращает обновлять параметры если ложить патроны игроку под ноги на старте уровня.
				-- self.ForceExitSearchAmmo = self.ForceExitSearchAmmo+1
				-- if self.ForceExitSearchAmmo>3 then self.ForceExitSearchAmmo=nil end
			-- end
			-- if not self.ForceExitSearchAmmo and Begin then self.ForceExitSearchAmmo=_time Begin=nil end
			-- if self.ForceExitSearchAmmo and _time>self.ForceExitSearchAmmo+3 then self.ForceExitSearchAmmo=nil end -- Эктив ловит и на этом прекращает обновлять параметры если ложить патроны игроку под ноги на старте уровня.
			-- if not ForceExitSearchAmmo and Begin then ForceExitSearchAmmo = 0 Begin=nil end
			-- if ForceExitSearchAmmo then -- Эктив ловит и на этом прекращает обновлять параметры если ложить патроны игроку под ноги на старте уровня.
				-- ForceExitSearchAmmo = ForceExitSearchAmmo+1
				-- if ForceExitSearchAmmo>4 then ForceExitSearchAmmo=nil self.ForceExitSearchAmmo=nil end
			--end
		end,
	...
	Dropped={
		OnBeginState = function(self)
			...
			-- Hud:AddMessage(self:GetName()..": Dropped")
			-- System:Log(self:GetName()..": NOT PHYSICS Dropped")
			self.hadFullSlots = nil
			self:Physicalize()
			self:SetTimer(1000)
		end,
	...
	Picked={
		----------------------------------
		OnBeginState = function(self)
			-- self.ForceExitSearchAmmo=nil
			-- System:Log("Picked/OnBeginState")
		...
		OnTimer = function(self)
			...
			if (self.ammopicked) then
				...
				-- System:Log("Picked/OnTimer/Inactive")
	...
	Inactive={
		...
		OnBeginState = function(self)
			-- self.ForceExitSearchAmmo=nil
			-- System:Log("Inactive/OnBeginState")
			AI:RegisterWithAI(self.id,0)
...
BasePickup.Client={
	OnInit = function(self)
		...
		-- if (self.floating_icon and self.Properties.bShowFloatingIcon==1 or tonumber(getglobal("game_DifficultyLevel"))==1) then
			self:LoadCharacter(self.floating_icon,0) -- Глючит, исправить!
		end
...
function __LoadAmmoIntoClips(self,collider,weaponid,ammo_type,ammo_amount) -- Рассмотреть каждый параметр чтобы наконец понять как работает.
...
--		Hud.cannot_pickup=1
	...
	if collider.ai then
		AI:Signal(0,1,"EXIT_SEARCH_AMMUNITION",collider.id)
	end
	return (amount-to_add)
end
------------------------------------------------------
function __PickWeapon(self,collider,entering) -- Противоположность: BasicWeapon.Server:Drop(Params)
	...
	if collider.cnt.weapon then
		SaveWeaponName = collider.cnt.weapon.name
	end
	-- Hud:AddMessage("__PickWeapon(self,collider,entering)")
	if count<4 and not has_weapon then -- has_weapon - если есть такое оружие.
		if (collider.cnt.MakeWeaponAvailable) then
			if not collider.ai and has_hands then
				collider.cnt:MakeWeaponAvailable(9,0)
			end
			collider.cnt:MakeWeaponAvailable(ClassID)
			if not collider.GunTableOfRandomTrace then collider.GunTableOfRandomTrace = {} end
			if not collider.GunTableOfRandomTrace[ClassID] then collider.GunTableOfRandomTrace[ClassID] = {} end
			if not collider.GunTableOfRandomTrace[ClassID].Color then collider.GunTableOfRandomTrace[ClassID].Color = {} end
			if self.RandomColor then -- self надо заменить на саму сущность оружия и тогда заработает правильно. Или это она и есть? Не могу проверить из-за того, что выброшенное оружие без физики вновь не поднимается.
				collider.GunTableOfRandomTrace[ClassID].Color = self.RandomColor
			else
				collider.GunTableOfRandomTrace[ClassID].Color = nil
			end
			if not collider.ai then
				if not collider.cnt.weapon or count==0 then
					-- System:Log(collider:GetName()..": not collider.cnt.weapon")
					collider.cnt:SelectFirstWeapon()
				else
					-- p_weapon_switch - автоматическое переключение оружия на подобранное.
					-- if (tonumber(p_weapon_switch)==1 or (self.hadFullSlots==1 and count==3)) then
					if (self.hadFullSlots==1 and count==3) or SaveWeaponName=="Hands" then
						collider.cnt:SetCurrWeapon(ClassID)
					end
				end
			-- elseif collider.ai and self.weapon then
			else
				-- BasicPlayer.AddItemInWeaponPack(collider,"Weapon",self.weapon)
				collider.cnt:SetCurrWeapon(ClassID)
				AI:Signal(0,1,"EXIT_SEARCH_AMMUNITION",collider.id)
			end
			-- local collider = collider.cnt
			AI:SoundEvent(collider.id,self:GetPos(),3,.5,.5,collider.id) -- Почему-то было убрано... Добавить и в оружие с физикой.
...
function CreateCustomPickup(params)
	...
	-- ret.pickuptype = "custom"
	if params.pickuptype then
		ret.pickuptype = params.pickuptype
	end
	return ret
end
