...
function SoundSpot:OnReset()
	...
	if (self.Properties.bPlay==0 and self.sound) then
		self:StopSound()
	end
...
SoundSpot["Client"] = {
	...
	OnInit = function(self)
		...
		if (self.Properties.bPlay==1) then
			self:PlaySound()
			--System:Log("Play sound"..self.Properties.sndSource)
		end
		self.Client:OnMove()
	end,
	----------------
...
----------------
function SoundSpot:PlaySound() -- Если тревога, то мутанты должны реагировать. Лучше сигналом.
...
--	if (self.sound and Sound:IsPlaying(self.sound)) then
	if (self.sound) then
		Sound:StopSound(self.sound)
...
function SoundSpot:LoadSnd()
	...
	if (self.Properties.sndSource~="") then
		...
		--System:Log("loading "..self.Properties.sndSource)
		Sound:SetSoundProperties(self.sound,self.Properties.fFadeValue)			
	end
