RandomAmbientSound = {
	...
	Properties = {
	    ...
	    fEnvSize=1,		--environment size in meters (win32 only) 
	    fEnvDiffusion=0,		--environment diffusion (win32/xbox) 
	    ...
	    fDecayTime=.1,		--reverberation decay time at mid frequencies (win32/xbox) 
	    fDecayHFRatio=.1,		--high-frequency to mid-frequency decay time ratio (win32/xbox) 
	    fDecayLFRatio=.1,		--low-frequency to mid-frequency decay time ratio (win32 only) 
	    nReflections=-10000,	--early reflections level relative to room effect (win32/xbox) 
	    fReflectionsDelay=0,	--initial reflection delay time (win32/xbox) 
	    ...
	    fReverbDelay=0,		--late reverberation delay time relative to initial reflection (win32/xbox) 
	    fReverbPan={x=0,y=0,z=0},		--late reverberation panning vector (win32 only) 
	    fEchoTime=.075,		--echo time (win32 only) 
	    fEchoDepth=0,		--echo depth (win32 only) 
	    fModulationTime=.04,	--modulation time (win32 only) 
	    fModulationDepth=0,	--modulation depth (win32 only) 
	    fAirAbsorptionHF=0,	--change in level per meter at high frequencies (win32 only) 
	    fHFReference=1000,	--reference high frequency (hz) (win32/xbox) 
	    fLFReference=20,		--reference low frequency (hz) (win32 only) 
	    fRoomRolloffFactor=0,	--like CS_3D_Listener_SetRolloffFactor but for room effect (win32/xbox) 
	    fDiffusion=0,		--Value that controls the echo density in the late reverberation decay. (xbox only) 
	    fDensity=0,		--Value that controls the modal density in the late reverberation decay (xbox only) 
...
function RandomAmbientSound:LoadSounds()
	for i,Element in self.Properties do
		...
		if (type(Element)=="table" and Element.sndSound) then
...
--			System:Log("doing Sound "..Element.sndSound)
				...
				if (Element.bCentered==1) then
					...
					--	System:Log(tostring(i)..".sndSound=nil")
					--end
					if (Element.sndSound~="") then
