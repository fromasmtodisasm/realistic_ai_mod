...
--		bodypos = 0,
	...
	DamageParticles1 = {--initial smoke that shows vehicle is damaged
		...
		speed = 1.5,
		count = 1,
		size = .06,size_speed=1.5,
		linearsizespeed=1,
		gravity={x=1,y=0,z=-.6},
		rotation={x=0,y=0,z=1.85},
		lifetime=4,
		turbulence_size=.62,
		turbulence_speed=5,
	...
	DamageParticles3 = {--fire created by burning vehicle after explosion
		...
		speed = .6, -- default .25
		count = 1, -- default 1
		size = 1.3,size_speed=.5, -- default size = .3-- default size_speed = .5
...
--	self:SetAICustomFloat(self.Properties.fFlightAltitude)
	...
	if (self.Properties.bSoundOutdoorOnly==1) then
		...
		self.sound_engine = Sound:Load3DSound(self.sound_engine_file,SOUND_OUTDOOR + SOUND_NO_SW_ATTENUATION,255,10,700) -- 255,5,700 - оригинал. 255,25,3000; 255,8,1000 - пытался подобрать.
		self.sound_engine_start = Sound:Load3DSound(self.sound_engine_start_file,SOUND_OUTDOOR + SOUND_RADIUS,255,10,700)
		self.sound_engine_stop = Sound:Load3DSound(self.sound_engine_stop_file,SOUND_OUTDOOR + SOUND_RADIUS,255,10,700)
	else
		self.ExplosionSound = Sound:Load3DSound(self.sound_explosion_file,SOUND_RADIUS,255,150,1000)
		self.sound_engine = Sound:Load3DSound(self.sound_engine_file,SOUND_NO_SW_ATTENUATION,255,10,700)
		self.sound_engine_start = Sound:Load3DSound(self.sound_engine_start_file,SOUND_RADIUS,255,10,700)
		self.sound_engine_stop = Sound:Load3DSound(self.sound_engine_stop_file,SOUND_RADIUS,255,10,700)
	end
...
function Gunship:AddGunner(puppet)
	...
	if self.gunnerT.entity then	-- already have a gunner
		do return 0 end
	end
...
--			AI:Signal(0,1,"READY_TO_GO",self.id)
		...
		if dropTarget then
			if (dropTarget.x<=helipos.x+5 and dropTarget.x>=helipos.x) and (dropTarget.y<=helipos.y+5 and dropTarget.y>=helipos.y) then
				self:InsertSubpipe(0,"h_ignore_collisions")
				atitude = helipos.z - dropTarget.z
			end
			--System:Log("ALT "..atitude.." dAld "..(self.Properties.dropAltitude+.4))
		else
			if self.CombatDropState then
				atitude = helipos.z - System:GetTerrainElevation(self:GetPos()) -- Добавить проверку на воду, если да, то приземлиться на точке атаки, иначе искать место.
			else
				System:Warning("[AI] helicopter Gunship:UpdateServer can't find pointReinforce")
			end
		end
		-- if not atitude then atitude="nil"  end
			-- Hud:AddMessage(self:GetName()..": atitude: "..atitude)
			-- System:Log(self:GetName()..": atitude: "..atitude)
		-- if atitude=="nil" then atitude=nil  end
		if (atitude and (atitude < (self.Properties.dropAltitude+.4) and atitude > 0)) then
			self:TrooperOut(dt)
		end
	end
...
function Gunship:OnDamageServer(hit)
	...
	if (hit.shooter==self.gunnerT.entity) then		-- no damage if shoont it self
--System:Log("Self Gunner damage")
		do return end
	end
...
--System:Log("Damage  "..hit.damage.." total "..self.damage)
	...
	local LevelName = Game:GetLevelName()
	if hit.explosion then
		-- self.damage = self.damage + hit.damage*(self.Properties.fDmgScaleExplosion)
		-- self.damage = self.damage + hit.damage*(.2)
		if (LevelName=="Training" and self:GetName()=="Gunship3" and self.PropertiesInstance.groupid==7231)
		or (LevelName=="Archive" and self:GetName()=="Gunship1" and self.PropertiesInstance.groupid==9872) then
			self.damage = 0
		elseif LevelName=="Boat" and self:GetName()=="Gunship0" and self.PropertiesInstance.groupid==487 then
			self.damage = self.damage + hit.damage*(.03)
		else
			self.damage = self.damage + hit.damage*(.2)
		end
	else
		-- self.damage = self.damage + hit.damage*(self.Properties.fDmgScaleBullet*.05)
		-- self.damage = self.damage + hit.damage*(.008)
		if (LevelName=="Training" and self:GetName()=="Gunship3" and self.PropertiesInstance.groupid==7231)
		or (LevelName=="Archive" and self:GetName()=="Gunship1" and self.PropertiesInstance.groupid==9872) then
			self.damage = 0
		elseif LevelName=="Boat" and self:GetName()=="Gunship0" and self.PropertiesInstance.groupid==487 then
			self.damage = self.damage + hit.damage*(.002)
		else
			self.damage = self.damage + hit.damage*(.008)
		end
	end

	-- Hud:AddMessage("self.damage: "..self.damage)

--System:Log("\001 Damage  "..self.damage.." prev "..prevDamage)
--System:Log("\001 maxAttack "..self.Properties.max_healthattack)
	if (self.damage > self.Properties.max_damageattack and prevDamage < self.Properties.max_damageattack) then

--System:Log("\001 low health >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
...
function Gunship:TrooperOut(dt)
	...
	System:Log(" Gunship trooper is out go OUT "..self.troopersNumber)
	...
	for idx=1,self.passengerLimit do
		if (self.passengersTT[idx].entity) then
			...
			self.passengersTT[idx].entity.HelicopterIs = nil

			-- if self.gunnerT and self.gunnerT.entity then
				-- Hud:AddMessage(self:GetName()..": self.gunnerT")
				-- local att_target = AI:GetAttentionTargetOf(self.gunnerT.entity.id)
				-- if att_target and type(att_target)=="table" then
					-- self.SenderId = att_target
				-- end
				-- if self.SenderId then
					-- self.passengersTT[idx].entity.CombatDropState = self.SenderId
					-- self.passengersTT[idx].entity:InsertSubpipe(0,"DropBeaconAt",self.SenderId)
					-- Hud:AddMessage(self:GetName()..": self.SenderId: "..self.SenderId)
				-- end
			-- end
...
--			self.troopers[idx]:SelectPipe(0,"h_user_getout")
			...
			self.troopersNumber = self.troopersNumber-1
			-- if self.CombatDropState and self.troopersNumber==1 then
				-- -- AI:Signal(0,1,"READY_TO_GO",self.id)
				-- AI:Signal(0,1,"READY_TO_GO",self.id)
				-- AI:Signal(0,1,"NEXTPOINT_ATTACK",self.id) -- Добавить задержку.
				-- -- AI:Signal(SIGNALFILTER_ANYONEINCOMM,1,"HEADS_UP_GUYS",self.id)
				-- self.dropState = 2
			-- end
			do return end
		end
...
function Gunship:OnCollideServer(hit)
	...
	if (hit.collier and hit.collider.IsCar==1) then -- Чтооо?
		self.damage = self.Properties.max_health+10
	end
...
--	wpnPos.z = wpnPos.z+1.4
	...
	self.mountedWeapon.Properties.angHLimit = -1
--	self.mountedWeapon.Properties.mountRadius = .1
