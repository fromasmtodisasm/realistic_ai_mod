...
--do return end
	...
	-- maximum damage
	-- local MaxHealth = self.Properties.max_health
	local MaxHealth = 150
	if (self.damage>MaxHealth) then
...
--MAX - add different damage particles here
	elseif (self.damage>MaxHealth*.75) then
	...
	elseif (self.damage>MaxHealth*.50) then
		-- midium damage (50%-75%)
		local vVec=self:GetHelperPos("vehicle_damage1",0)
		Particle:CreateParticle(vVec,g_Vectors.v001,self.DamageParticles1) --vector(0,0,1) is up
		Particle:CreateParticle(vVec,g_Vectors.v001,self.DamageParticles1a) --vector(0,0,1) is up
	elseif (self.damage>MaxHealth*.25) then
		-- small damage (25%-75%)
		local vVec=self:GetHelperPos("vehicle_damage1",0)
		Particle:CreateParticle(vVec,g_Vectors.v001,self.DamageParticles1) --vector(0,0,1) is up
		Particle:CreateParticle(vVec,g_Vectors.v001,self.DamageParticles1a) --vector(0,0,1) is up
...
--	ExecuteMaterial(self:GetPos(),explDir,HeliExplosion,1)
	local SoundTable1
	if self.landed==0 then
		SoundTable1 = {
			"Sounds/Explosions/generic_air_explosion.mp3",
			"Sounds/Explosions/generic_air_explosion_02.mp3",
		}
	else
		SoundTable1 = {
			"Sounds/Explosions/generic_helicopter_explosion.mp3",
		}
	end
	if SoundTable1 then
		local snd = Sound:Load3DSound(SoundTable1[random(1,getn(SoundTable1))],SOUND_RADIUS,255,30,1000)
		if Hud and Hud.SharedSoundScale~=0 then
			if snd then	self:PlaySound(snd)	end
		end
	end
	Particle:SpawnEffect(pos,explDir,self.ExplosionEffect)

	-- raduis,r,g,b,lifetime,pos
	CreateEntityLight(self,30,1,1,2,.5) -- 7,1,1,.7,.5 - Оригинал
	...
	-- -- if nDamage>600 then
	-- if nDamage~=600 then -- Надоели вертолёты-пушинки.
		-- nDamage=600
	-- end
	...
	ExplosionParams.damage = 600
	ExplosionParams.rmax = 70.5
	ExplosionParams.rmin = 12
	ExplosionParams.radius = 30  -- Урон.
	ExplosionParams.impulsive_pressure = 600  -- Имульс.
...
--	end
	...
	-- AI:FreeSignal(-1,"WakeUp",pos,150,_localplayer.id)
	-- AI:FreeSignal(-1,"WakeUp2",pos,150,_localplayer.id)
	AI:SoundEvent(self.id,pos,300,1,0,_localplayer.id)
...
--	--do sounds
	...
	if (self.part_time >.01) then		------not every frame update
		HC.ExecuteDamageModel(self)
		self.part_time = 0
	end

	-- -- Для вертолёта Кроу придумать что-нибудь другое.
	-- if self.Properties.fDmgScaleBullet~=.004 then
		-- self.Properties.fDmgScaleBullet = .004
	-- end
	-- if self.Properties.fDmgScaleExplosion~=.2 then
		-- self.Properties.fDmgScaleExplosion = .2
	-- end
