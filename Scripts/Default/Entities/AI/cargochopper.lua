...
-- Рг не могут уничтожить этот вертолёт
...
--	theDrop3 = nil,
	...
	Properties = {
		...
		forward_speed = 10,
		back_speed = 5,
		max_health = 150, -- Скорректировать!
		accuracy = .6,
		responsiveness = 5,
		species = 1,
		fSpeciesHostility = 2,
...
--	self:SpawnStuff()
	...
	if (self.Properties.bSoundOutdoorOnly==1) then
		...
		self.sound_engine = Sound:Load3DSound(self.sound_engine_file,SOUND_OUTDOOR + SOUND_NO_SW_ATTENUATION,255,10,700)
		...
		self.sound_engine = Sound:Load3DSound(self.sound_engine_file,SOUND_RADIUS,255,10,700)
...
--	System:Log(self.PropertiesInstance.Rope1Name)
...
--System:Log("Damage  "..hit.damage.." total "..self.damage)
	...
	if (hit.explosion) then
		...
		self.damage = self.damage + hit.damage*(.18)
	else
		-- self.damage = self.damage + hit.damage*(self.Properties.fDmgScaleBullet*.05)
		self.damage = self.damage + hit.damage*(.006)
	end
...
System:Log("CargoChopper OnShutDOWN ---------------------------")
...
--System:Log("\001 CargoChopper RopeDown ")
...
--	end
	...
	self.theRopeEntity1:Down(0)
	self.theRopeEntity2:Down(.8)
	self.theRopeEntity3:Down(.3)
...
--System:Log(" CargoChopper:DropDone  -------------------------------------------------->>>")
...
--System:Log("\001 CargoChopper:ApproachingDropPoint  -------------------------------------------------->>>")
...
--System:Log("\001 CargoChopper:TrooperOut  found			>>>")
...
--System:Log("\001 CargoChopper:TrooperOut  ######>>>")
--	self.troopersNumber = 0
