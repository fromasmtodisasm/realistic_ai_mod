...
local ExplodeClient=function(self)
	-- hide grenade
	self:DrawObject(0,0)
	local canseeflash = PlayerCanSee(self,_localplayer,.3)
	if (canseeflash) then
	...
	if (players and type(players)=="table") then
		for i,cell in players do
			...
			if (tempplayer.ai) then
				...
				local dot = -1
				if Difficulty>=2 then -- >=4
					dot = .3  -- "Ослепляются" только если смотрели на гранату.
...
local param={
	...
	flashTime = 20, -- Время ослепления может достигать 20-30 секунд. Добавить: Сделать чтобы зависело от расстояния.
	...
	-- ExplosionParams = {
		...
		-- radius = 1,
		-- DeafnessRadius = 25,
		-- DeafnessTime = 50,  -- Время потери слуха может достигать 4-6 часов, но это игра, поэтому не стоит ставить большие значения.
		-- impulsive_pressure = 0,
		-- shooter = nil,
		-- weapon = nil,
		-- rmin_occlusion=.2,
		-- occlusion_res=32,
	...
	light = {
		fRadius = 40,
		vDiffRGBA = {r = 1,g = 1,b = 1,a = 1,},
		vSpecRGBA = {r = .3,g = .3,b = .3,a = 1,},
		fLifeTime = 1,
	},
}

FlashbangGrenade=CreateHandGrenade(param) -- Оглушение не работает потому что что-то связано вот с этим.
