...
OICWSP = {
	...
	AimOffset={x=.178, y=.2, z=.058},					--IronSights
	---------------------------------------------------
	PlayerSlowDown = .8,									-- factor to slow down the player when he holds that weapon
	---------------------------------------------------
	-- ActivateSound = Sound:LoadSound("Sounds/Weapons/usrif_g36c/g36c_deploy_1p.mp3",0,155),	-- sound to play when this weapon is selected
	...
	-- if (Game:IsMultiplayer()) then
		-- if (_localplayer.cnt.firemode==1) then
			-- if (_localplayer.cnt.vTargetSpot and _localplayer and _localplayer.cnt.aiming==1) then
				...
				-- local iTargetAngle = self.cnt:GetProjectileFiringAngle(65,3*9.8,fDistX,fDistY)
			...
			-- --	printf("xdist=%.2f ydist=%.2f",fDistX,fDistY)
				...
				-- if (elapstime < self.minaimtime or iTargetAngle==0
					...
					-- Game:WriteHudString(425,280,"wait="..sprintf("%.2f",self.minaimtime - elapstime),.7,.7,.7,1,15,15)

				-- else
					-- System:DrawImageColor(self.TargetHelperImage,400 - 15,iHeight - 15,30,30,4,1,0,0,1)
					-- Game:WriteHudString(425,300,"y="..sprintf("%.2f",self.initangle - vAngles.z),.7,.7,.7,1,15,15)
					-- Game:WriteHudString(425,320,"x="..sprintf("%.2f",iAngleDiff),.7,.7,.7,1,15,15)
					-- Game:WriteHudString(425,340,"dist="..sprintf("%.2f",fDistX),.7,.7,.7,1,15,15)
				...
				-- if (myPlayer) then
					...
					-- local	lTargetSpot = {x=0,y=0,z=0}
						...
						-- if (TargetDescTable) then
							...
							-- iTargetAngle = self.cnt:GetProjectileFiringAngle(65,3*9.8,fDistX,fDistY)
						-- end
					-- if (_localplayer.cnt.aiming) then
						-- if (iTargetAngle==0) then
							-- System:DrawImageColor(self.NoTargetImage,400 - 15,300 - 15,30,30,4,1,0,0,1)
							-- local elapstime =_time-_localplayer.cnt.aimtime
