...
function BaseHandGrenade:OnReset()
	-- if self.exploding then
		-- Sound:StopSound(self.exploding) -- Не помогает.
	-- end
end
...
function BaseHandGrenade:Client_OnInit()
	...
	if (self.model) then
		...
		if (self.model_nooffset==nil) then
			self:SetObjectPos(0,{x=0,y=0,z=-.07})
		end
 	end
	...
	if (self.softcover and not Game:IsMultiplayer()) then
		self:LoadObject(self.softcover,1,1)
		self:DrawObject(1,0)
	end
...
function BaseHandGrenade:Server_Flying_OnUpdate(dt)
	...
	if self.AITargetType_ALTERNATE==AIAnchor.AIOBJECT_DAMAGEGRENADE then
		AI:FreeSignal(-1,"OnGrenadeSeen_Flying",pos,15,self.id)
	end
	-- bouncing AI sound
	if (status) then
		...
		if self.AITargetType==AIAnchor.AIOBJECT_DAMAGEGRENADE or self.AITargetType_ALTERNATE==AIAnchor.AIOBJECT_DAMAGEGRENADE then
			-- AI:FreeSignal(-1,"OnGrenadeSeen_Flying",pos,sound.SoundRadius,self.id)
			AI:FreeSignal(-1,"OnGrenadeSeen_Colliding",pos,12,self.id)
		end

		-- apply some damage to the AI guy
		-- if (self.damage_on_player_contact and status.IsPlayer and status.target) then -- Маразм. Ладно если кирпич башка попадёт, или граната с огромной высоты, но чтобы так...
		...
		if (self.AITargetType) then -- Оригинал
		...
		-- if (self.AITargetType) then
			-- if (self.AITargetType==AIOBJECT_ATTRIBUTE or self.AITargetType==AIAnchor.AIOBJECT_DAMAGEGRENADE) then
				-- if (self.AITargetType_ALTERNATE==AIOBJECT_ATTRIBUTE) then
					-- local shooter = self.saveshooter
					-- AI:RegisterWithAI(self.id,self.AITargetType_ALTERNATE,shooter.id)
					-- self.AITargetType = self.AITargetType_ALTERNATE
				-- elseif (self.AITargetType_ALTERNATE) then
					-- AI:RegisterWithAI(self.id,self.AITargetType_ALTERNATE)
					-- self.AITargetType = self.AITargetType_ALTERNATE  -- dont do it again
				-- end
			-- end
...
function BaseHandGrenade:Client_Flying_OnUpdate(dt)
	if tonumber(getglobal("game_DifficultyLevel"))<2 then
		...
		if (not self.no_trail and not self.was_underwater) then
			...
			if (lenSq > 6 and self.SmokeTrail and self.part_time>.015) then
				local pos = self:GetPos()
	...
	if (not self.was_underwater) then
		if (Game:IsPointInWater(pos)) then
			...
			local status=GetParticleCollisionStatus(self)
			if (status and ((_time-self.last_bounce)>.2)) then
...
--	if ((self.dynamic_light>0) and self.bExplosionLightAdded==0) then
	if not self.bExplosionLightAdded and not self.no_light then
		-- raduis,r,g,b,lifetime
		CreateEntityLight(self,10,1,1,.3,.5)
--		-- vPos,fRadius,DiffR,DiffG,DiffB,DiffA,SpecR,SpecG,SpecB,SpecA,fLifeTime
--		self:AddDynamicLight(self:GetPos(),4,1,1,.3,1,1,1,.3,1,2)
...
function BaseHandGrenade:Launch(weapon,shooter,pos,angles,dir,lifetime)
	...
	--System:Log("--> Lauch position==x: "..pos.x.." y:"..pos.y.." z:"..pos.z)

	-- self.saveshooter = shooter
...
BaseHandGrenade.Server = {
	...
	Exploding = {
		OnBeginState = function(self)
			self:SetTimer(self.lifetime)
			AI:FreeSignal(-1,"PRE_GRENADE_EXIT",pos,999,self.id)
	...
	Exploding = {
		OnBeginState = function(self)
			...
			if (not self.no_explosion) then
				...
				local sound = self.AIExplodingSound
				-- AI:FreeSignal(-1,"WakeUp",hit.pos,sound.SoundRadius/2,self.id)
				-- AI:FreeSignal(-1,"WakeUp2",hit.pos,sound.SoundRadius/2,self.id)
				if _localplayer.id then -- Как-то nil при загрузке выдало.
				...
				if (material and material[effect]) then -- material выдавал nil
			...
			if (self.softcover and not Game:IsMultiplayer()) then
				self:DrawObject(0,0)
				self:CreateStaticEntity(1,Game:GetMaterialIDByName("mat_smoke"),1)
