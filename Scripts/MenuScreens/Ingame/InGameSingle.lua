		...
		OnActivate = function(Sender)
			...
			for x,SaveGame in SaveList do
				if (strlower(tostring(SaveGame.Level))==strlower(tostring(szCurrentLevel))) then
					...
					if getglobal("g_language") and (strlower(getglobal("g_language"))=="russian" or strlower(getglobal("g_language"))=="french") then
			...
			if iLastCheckpoint then
				...
				if _localplayer and _localplayer.IsAiPlayer and _localplayer.cnt.health <= 0 and _localplayer.AllowAutoLoadSave then
					_localplayer.AllowAutoLoadSave = nil
					UI.PageInGameSingle.GUI.LoadGame.OnCommand(Sender)
					-- System:Log("UI.PageInGameSingle.GUILoadGame.OnCommand(Sender)")
				end
			end
			UI.PageInGameSingle:SetPicture()
			if Hud then
