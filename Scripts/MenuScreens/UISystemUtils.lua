...
function UI:CheckCutSceneDrive()
	...
	for i,szFileName in FileList do
		if (strlower(strsub(szFileName,-4))==".bik") or (strlower(strsub(szFileName,-4))==".avi") then -- Исправление, чтобы также и ролики миссий воспроизводило.
...
function UI:PlayMusic()
	if (UI.MusicId and tostring(getglobal("s_MusicEnable"))=="1") then
		Sound:SetSoundLoop(UI.MusicId,1)
		Sound:PlaySound(UI.MusicId)
		Sound:SetSoundVolume(UI.MusicId,getglobal("s_MusicVolume") * 255)
	end
end
...
function UI:AddChatbox(Page,x,y,w,h,inputh,teambased)
	...
	Page["PopulateChatTarget"] = function(Sender,bRepopulating)
		...
		Sender.fNextRepopulateTime = _time + .25
...
function DisconnectOnYes()
	-- System:Log("DisconnectOnYes!!!")
	GotoPage("Disconnect") -- function ClientOnDisconnect(szReason) - C++
	-- local BackVideo = tonumber(getglobal("ui_BackGroundVideo")) -- Ничего не дало.
	-- if BackVideo==0 then
...
function GotoPage(PageName,Back,HideBackground)
	...
	if HideBackground and HideBackground~=0 then
		...
		if tonumber(getglobal("ui_BackGroundVideo"))~=0 then
			...
			UI:ShowWidget(UI.PageBackScreen.GUI.Video) -- То, что выше, оно не помогает.
			UI:HideWidget(UI.PageBackScreen.GUI.StaticImage)
		else
			UI:DisableWidget("Video","BackScreen")
			UI:HideWidget("Video","BackScreen")
			if not ClientStuff or (_localplayer and _localplayer.cnt.health==0) then -- Показывать менюшную картинку во время игры только когда мёртв или когда только запустился.
				UI:ShowWidget(UI.PageBackScreen.GUI.StaticImage)
			else
				UI:HideWidget(UI.PageBackScreen.GUI.StaticImage)
			end
...
function AddUISideMenu(Page,ItemList)
	...
	for ItemName,ItemTable in ItemList do
		...
		if ((ItemTable[1]=='-') and (ItemTable[2]=='-') and (ItemTable[3]=='-')) then
			iStartY = iStartY + (UI.skins.SideMenuButton.height * .5) + iSpacing * .5
		else
			Page[ItemTable[1]] =
			{
