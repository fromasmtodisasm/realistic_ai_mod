	...
	GUI=
	{
		...
		OnUpdate = function(Sender)
			...
			if (UI.PageCutScenePlayer.bCanSkip and tonumber(UI.PageCutScenePlayer.bCanSkip)~=0) then
				if (_time - UI.PageCutScenePlayer.fStartTime) >= UI.fCanSkipTime then
					...
					if szKeyName=="esc"  or szKeyName=="spacebar" or szKeyName=="lshift" or szKeyName=="rshift" or szKeyName=="f7" then
						UI:StopCutScene()
						if UI.PageCutScenePlayer.pfnFunction then
							UI.PageCutScenePlayer.pfnFunction() -- Это чтобы при нажатии на кнопку сразу включалось меню, а не следующий ролик.
						end
					end
				end
			end
		...
		OnActivate= function(Sender)
			...
			if (UI.PageCutScenePlayer.szCutSceneName) then
				...
				UI:HideMouseCursor()
				UI:HideBackground()
				UI.PageCutScenePlayer.AllowPlayMusicOnStop = 1 -- Это если ролик так и не был найден, вызвалось Finished(), то музыка после не заиграет.
		...
		OnDeactivate = function(Sender)
			...
			if UI.PageCutScenePlayer.AllowPlayMusicOnStop or not HUD then
				UI:PlayMusic() -- Это ещё и музыка в меню, когда только включил игру.
			end
		end
	},
}
...
function UI.PageCutScenePlayer:Finished()
	UI.PageCutScenePlayer.AllowPlayMusicOnStop = nil
...
function UI:PlayCutSceneEx(szCutSceneName,szMessage,szGotoPage,bGotoPageShowBack,bCanSkip)
	...
	Game:ShowMenu() -- Когда ролики появятся, раcкомментировать и проверить дамбу.
