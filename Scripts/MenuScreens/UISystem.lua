...
function UI:OnInit()
	...
	if getglobal("g_language") and strlower(getglobal("g_language"))=="russian" then
		Language:LoadStringTable("MenuTable_ru.xml")
	else
		Language:LoadStringTable("MenuTable_en.xml")
	end
	...
	Script:LoadScript(szScriptFolder.."Options/GameMod.lua",1)
...
-- System Idle
--------
function UI:OnIdle(fIdleTime) -- C++
	-- 60 seconds of idle before doing something
	if (fIdleTime > 120) then
		...
		if ClientStuff and not UI.bInGameOverride then
			...
			if not UI.PageBackScreen.GUI.Video:IsPlaying() then
				if ClientStuff.GetInGameMenuVideoOn then
					if ClientStuff:GetInGameMenuVideoOn()~=0 and _localplayer and _localplayer.cnt.health==0 then
						UI.PageBackScreen:PlayRandomVideo() -- Проигрывать видео на фоне только когда мёртв. Связано с PlayRandomVideo функцией в OnActivate. Тоже может воспроизводить при переключении в меню, но эта ещё и после смерти это делает.
					end
				end
			end
			local iVideoOn = 1
...
--			System:Log("UI:OnSwitch MenuPage="..sMenuPageName)		-- debugging
			if not Game:IsMultiplayer() then
				if _localplayer and _localplayer.cnt.health > 0 then
					GotoPage(sMenuPageName,1,1) -- Это влияет только на меню, которое появляется при нажатии кнопки паузы во время игры.
				else
					if iVideoOn~=0 then
			...
			-- if _localplayer.cnt.health > 0 and iVideoOn==0 then -- Не убирать комментирование.
				-- GotoPage(sMenuPageName,1,1)
			-- elseif (iVideoOn~=0) then
				-- GotoPage(sMenuPageName,1,0)
			-- end
...
------
function UI:CanRenderGame() -- Вот блин, а я так долго это искал! C++
	if not ClientStuff then return nil end
	-- Если погиб в игре, то не рисовать на фоне игру.
