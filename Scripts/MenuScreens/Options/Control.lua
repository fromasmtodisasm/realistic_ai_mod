	...
	GUI=
	{
		...
		sensitivity=
		{
			...
			OnChanged = function(Sender)
				Input:SetMouseSensitivity(Sender:GetValue()*50+5)
			end,
		},
		...
		mousesmoothing=
		{
			...
			OnChanged = function(Sender)
				...
				if (curSelection==1) then
					setglobal("i_mouse_smooth","0")
				elseif (curSelection==2) then
					setglobal("i_mouse_smooth",".5")
				elseif (curSelection==3) then
					setglobal("i_mouse_smooth","1")
				end
			end,
		},
		...
		mouseacceleration_enable =
		{
			...
			OnChanged = function(Sender)
				if (UI.PageOptionsControl.GUI.mouseacceleration_enable:GetChecked()==nil) then
					UI.PageOptionsControl.GUI.mouseacceleration:SetValue(0)
					setglobal("i_mouse_accel","0")
					UI:DisableWidget(UI.PageOptionsControl.GUI.mouseacceleration)
				else
					UI.PageOptionsControl.GUI.mouseacceleration:SetValue(.1)
					setglobal("i_mouse_accel",".1")
		...
		OnActivate= function(Sender)
			...
			-- sensitivity
			UI.PageOptionsControl.GUI.sensitivity:SetValue((Input:GetMouseSensitivity()-5)/50)
			...
			if (cur_i_mouse_smooth==.5) then
				UI.PageOptionsControl.GUI.mousesmoothing:SelectIndex(2)
			elseif (cur_i_mouse_smooth==1) then
...
function UI.PageOptionsControl:AddBind(Action)
	...
	-- System:Log("ID: "..b1) -- 24 - 27 - слот 1 - слот 4.
	-- if b1=="@control55" then -- Это кнопка W. Где же найти весь список id кнопок?
		-- b1=""
	-- end
	if (Binding[2]) then
...
function UI.PageOptionsControl.FillBindList()
	...
	for szType,Map in ActionMap do
		...
		-- System:Log("szType: "..szType) --  Заголовки блоков, что в самом верху этого файла находятся.
		-- System:Log("szTypeTitle: "..szTypeTitle)  -- Собственно заголовки блоков с припиской @CONTROLS_.
		-- System:Log("MapOrder: "..MapOrder[1]) -- По четыре строчки пунктов меню. Move_forward, use и т.п.
		UI.PageOptionsControl.GUI.controllist:AddItem(szTypeTitle)
		for i,szActionName in MapOrder do
			for j,Action in Map do
				-- System:Log("szActionName: "..szActionName) -- Пункт меню.
				...
				if (strlower(strsub(Action.desc,2))==strlower(szActionName)) or
					...
					-- System:Log("Action.desc: "..strlower(strsub(Action.desc,1))) -- Вот такое название: @slot4 И Action.desc - то же самое.
					local iIndex = UI.PageOptionsControl:AddBind(Action)
					-- System:Log("iIndex: "..iIndex) -- Номер по порядку.
					if (iIndex) then
...
function UI.PageOptionsControl.ResetToDefaults()
	...
	UI.PageOptionsControl.GUI.sensitivity:SetValue(.35)
	UI.PageOptionsControl.GUI.mousesmoothing:SelectIndex(1)
	UI.PageOptionsControl.GUI.mouseacceleration_enable:SetChecked(0)
