	...
	GUI=
	{
		...
		soundfxvolume=
		{
			...
			OnChanged = function(Sender)
				...
				if (sound) then
					Sound:SetSoundVolume(sound,getglobal("s_SFXVolume") * 255)
		...
		musicvolume=
		{
			...
			OnChanged = function(Sender)
				...
				Sound:SetSoundVolume(UI.MusicId,Sender:GetValue() * 255)
			end,
		},
...
function UI.PageOptionsSound.ResetToDefaults()
	UI.PageOptionsSound.GUI.musicvolume:SetValue(.6)
	UI.PageOptionsSound.GUI.soundfxvolume:SetValue(1)
