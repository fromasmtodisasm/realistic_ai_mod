...
UI.PageCampaignStart=
{
	GUI=
	{
		...
		-- {
			...
			-- OnCommand=function(Sender)
				-- if tonumber(getglobal("game_CurrentCampagin"))==1 then
					-- UI.PageCampaignStart.szLevelName = "Training Training" 		-- is used by YesNoBox command
				-- elseif tonumber(getglobal("game_CurrentCampagin"))==2 then
					-- UI.PageCampaignStart.szLevelName = "SpecOps01 SpecOps01"
				-- end
		...
		SelectCampagin=
		{
			...
			OnChanged = function(Sender)
				local sel = UI.PageCampaignStart.GUI.SelectCampagin:GetSelectionIndex()
				setglobal("game_CurrentCampagin",sel) -- По умолчанию 1. Во время игры не менять, пользы это не даст.
				System:Log("UI.PageCampaign:SelectCampagin: "..getglobal("game_CurrentCampagin"))
			end,
		},
		...
		OnActivate = function(Sender)
			...
			UI.PageCampaignStart.iDifficultyLevel = 2  -- 0 - Рамка на первом уровне. -- 1
			...
			-- end
			local Difficulty = tonumber(UI.PageCampaignStart.iDifficultyLevel)
			if Difficulty<2 then
				UI.PageCampaignStart.GUI.Easy:SetChecked(1)
				UI.PageCampaignStart.GUI.Easy:OnChanged()
			elseif Difficulty<4 then
				UI.PageCampaignStart.GUI.Hard:SetChecked(1)
				UI.PageCampaignStart.GUI.Hard:OnChanged()
			else
				UI.PageCampaignStart.GUI.Realistic:SetChecked(1)
				UI.PageCampaignStart.GUI.Realistic:OnChanged()
			end
			...
			-- end
			UI.PageCampaignStart.GUI.SelectCampagin:Clear()
			UI.PageCampaignStart.GUI.SelectCampagin:AddItem(Localize("Original_campaign"))
			UI.PageCampaignStart.GUI.SelectCampagin:AddItem(Localize("Spetsnaz_Bull_Skull"))
			UI.PageCampaignStart.GUI.SelectCampagin:SelectIndex(1)
		end
	},
	...
	StartGameReally = function()
		if tonumber(getglobal("game_CurrentCampagin"))==1 then
			setglobal("p_model","objects/characters/pmodels/hero/hero.cgf")
		elseif tonumber(getglobal("game_CurrentCampagin"))==2 then
			setglobal("p_model","objects/characters/specops/merc/hero.cgf")
		end
		if (UI.iStat) then
			setglobal("g_LevelStated",1)
		else
		...
		if (strlower(UI.PageCampaignStart.szLevelName)=="training training") then
			UI:PlayCutScene("training_begin.bik","StartLevel "..UI.PageCampaignStart.szLevelName) -- Доделать.
		elseif (strlower(UI.PageCampaignStart.szLevelName)=="specops01 specops01") then -- Название уровня должно быть в нижнем регистре!
			UI:PlayCutScene("SpecOps.bik","StartLevel "..UI.PageCampaignStart.szLevelName)
		else
			Game:SendMessage("StartLevel "..UI.PageCampaignStart.szLevelName)
		end
		-- System:Log("szLevelName: "..strlower(UI.PageCampaignStart.szLevelName))
		...
		if (iSelection) then
			...
			if (CheckpointList) then
				...
				for i=1,iCPCount do
					...
					if (getglobal("g_language") and strlower(getglobal("g_language"))=="french" or strlower(getglobal("g_language"))=="russian") then
...
function UI.PageCampaign:SetAIDifficulty(DifficultyLevel)
	...
	-- if tonumber(getglobal("game_DifficultyLevel"))==0 then		-- EASY
		...
		-- -- setglobal("game_Accuracy",.4)
		-- -- setglobal("game_Aggression",.4)
		-- -- setglobal("game_Health",.5)
		-- -- setglobal("ai_allow_accuracy_decrease",1)
		-- -- setglobal("ai_allow_accuracy_increase",0)
		-- -- setglobal("ai_SOM_SPEED",1.2)
		-- -- setglobal("hud_damageindicator",1) -- Показать индикатор повреждений (красная рамка).
	-- elseif tonumber(getglobal("game_DifficultyLevel"))==1 then	-- MEDIUM
		-- System:Log("UI.PageCampaign:SetAIDifficulty medium")
		-- -- setglobal("game_Accuracy",.5)
		-- -- setglobal("game_Aggression",.5)
		-- -- setglobal("game_Health",.7)
		-- -- setglobal("ai_allow_accuracy_decrease",1)
		-- -- setglobal("ai_allow_accuracy_increase",1)
	...
	-- elseif tonumber(getglobal("game_DifficultyLevel"))==2 then	-- CHALLENGING
		...
		-- -- setglobal("game_Accuracy",.6)
		-- -- setglobal("game_Aggression",.6)
		-- -- setglobal("game_Health",1)
		-- -- setglobal("ai_allow_accuracy_decrease",1)
		-- -- setglobal("ai_allow_accuracy_increase",1)
		-- -- setglobal("ai_SOM_SPEED",1.5)
	-- elseif tonumber(getglobal("game_DifficultyLevel"))==3 then	-- BRUTAL
		...
		-- -- setglobal("game_Accuracy",.8)
		-- -- setglobal("game_Aggression",.8)
...
function Game:DisplayTimeZone()
	if not ClientStuff or not AI or not UI then	return end
...
AddUISideMenu(UI.PageCampaign.GUI,
{
	...
	{"StartGame",Localize("Start"),UI.PageCampaignStart.GUI.StartGame.OnCommand,},
})
