...
function ScoreBoardManager:RenderTeam(team,score,xpos,ypos,xsize,ysize,clr,bspectator,bclass,yScale)	
	...
	if (bspectator and count(team)>=1) then			
		...
		%Game:WriteHudString(400-size*.5,tpos,"@ScoreBoardSpectators",1,1,1,1,textsizeScaled,textsizeScaled)
		tpos=tpos+10*yScale 				
	end
	...
	for i,val in team do														
		if (not bspectator) then
			...
			if (bclass and val.class) then				
				...
				local offset=((fieldScaledSizeList[0]-fieldSpaceScaledSize)-textureScaled)*.5 										
				...
				for v,item in fieldSizeList do											
			  		...
			  		if (val[currOffset]) then
			  			local fieldVal=val[currOffset] 
							...
							%Game:WriteHudString(xoffset+fieldx-(fieldScaledSizeList[currOffset]+textSize+fieldSpaceScaledSize)*.5,tpos+6*yScale,text,1,1,1,1,textsizeScaled,textsizeScaled)
			...
			-- current spectator
			if (team[i+1]) then							
				%Game:WriteHudString(currSpecPosX+400-maxSpecPosX*.5,tpos,entName..",",1,1,1,1,textsizeScaled,textsizeScaled)			
...
function ScoreBoardManager:RenderTeamGame(bclass)
	...
	if (self.visible==1 and ScoreBoardManager.tblScoreList) then
	...
	--		return 
		...
		local ypos=300-(((playersCount+spectatorsCount)+3)*21)*.5*yScale 		
		...
		-- display teams data
		local bluepos=self:RenderTeam(teams.blue,Game:GetTeamScore("blue"),45,ypos,256*1.4,ysize,{0,0,1,.3},nil,bclass,yScale)
		local redpos=self:RenderTeam(teams.red,Game:GetTeamScore("red"),410,ypos,256*1.4,ysize,{1,0,0,.3},nil,bclass,yScale)
...
function ScoreBoardManager:RenderTeamWindow(xpos,ypos,playersCount,spectatorsCount,r,g,b,xScale,yScale)
	...
	Hud:DrawScoreBoard(x,y,xScale,.45*yScale,100,Hud.tmpscoreboard.bar_score,r,g,b,1,0,0)		
	y=y+31*yScale 
...
function ScoreBoardManager:RenderDMGame()
	...
	if (self.visible==1 and ScoreBoardManager.tblScoreList) then
		...
		-- display scoreboard
		ScoreBoardManager:RenderServerInfo(400-256*1.38*.5,ypos-10*yScale,0,0,1,1.38,yScale)
		ScoreBoardManager:RenderTeamWindow(400-256*1.38*.5,ypos-10*yScale,playersCount,spectatorsCount,0,0,1,1.38,yScale)
							
		-- display teams data
		ScoreBoardManager:RenderTeam(players,nil,400-256*1.38*.5+5,ypos,256*1.38,ysize,{0,0,1,.3},nil,0,yScale)
