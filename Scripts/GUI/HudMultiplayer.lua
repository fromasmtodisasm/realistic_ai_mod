...
function Hud:DrawCTFTeamScore(x,y,otherteamstate,team)
	...
	%System:DrawImageColor(self.white_dot,x,y,30,30,4,tclr[1],tclr[2],tclr[3],.4)
	if (otherteamstate) then
		%System:DrawImageColor(self.flag_icon,x,y,30,30,4,.7,.7,.7,.5)
	else
...
function Hud:DrawTeams(player)
	...
	--if (team=="red") then
--		%System:DrawImageColor(self.white_dot,0,559,800,600,4,1,0,0,.4)
	--else
	--	%System:DrawImageColor(self.white_dot,0,559,800,600,4,0,0,1,.4)
	--end

	local red_score=Game:GetTeamScore("red")
	local blue_score=Game:GetTeamScore("blue")

	--%System:DrawImageColor(self.white_dot,0,0,50,30,4,1,0,0,.4)
	--%Game:WriteHudNumber(5,0,red_score,1,1,1,1,30,30)
	--%System:DrawImageColor(self.white_dot,0,31,50,30,4,0,0,1,.4)
...
function Hud:DrawCrosshairName(player) -- А режиме штурма имена простых ботов не отображаются, player не передаётся...

	if not player or not player.cnt or not player.cnt.GetViewIntersection then return end
