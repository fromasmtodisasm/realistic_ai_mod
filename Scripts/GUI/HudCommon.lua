Hud={
	...
	color={r=.66,g=.8,b=.8},
	color_top={r=.66,g=1,b=1},
	color_red={1,0,0,1},--color_red={1,.3,.3,1},previous red..
	color_blue={.1,.6,.75,1},
	...
	Progress=
	{
		Player=							-- player progress indicator
		{
			...
			Seconds=1,			-- amount of seconds this indicator should stay (nil=forever)
...
Hud.Progress.Player.Draw = function(hud)
	...
	local fStep=42/70
	hud:DrawScoreBoard(400-(strwidth*.5)-10,ypos,size,fStep,100,hud.tmpscoreboard.bar_score,1,1,1,1,0,0)
	ypos=ypos+42
	hud:DrawScoreBoard(400-(strwidth*.5)-10,ypos,size,1,100,hud.tmpscoreboard.bar_bottom,1,1,1,1,0,0)
	...
	if (PTable.ValueX) then				-- progress was given
		...
		g = 1 + (g-1) * progressStep
		if (g>=1) then g=1  end
		r = r + (1-r) * progressStep
		if (r>=1) then r=1  end

		hud:DrawBar(400-(strwidth*.5)-5,320,progressStep*strwidth*1.28,32,35,g,r,0,.8)

	else												-- no progress was given (e.g. cannot build because construction are is blocked)

		hud:DrawElement(395,ypos-10,hud.pickups[19], 1,1,1,.9)

	end

	Game:WriteHudString(400-(strwidth*.5)+10,300,text, 1,1,1,1,26,26)			    -- information text
end
...
Hud.Progress.AssaultState.Draw = function(hud)
	...
	while  iItemCurrent < iItemCount do
		if (iItemCurrent	==tonumber(PTable.ValueX)) then
			...
			local itemScale,itemMaxScale=.66,.85

			-- state has changed ?
			if (hud.ProgressStateTime~=0) then
				...
				itemScale=.66*(invSqrScaleAmount+scaleSqrAmount)+2*itemMaxScale*invScaleAmount*hud.ProgressStateTime

				-- reset animation
				if (hud.ProgressStateTime<0) then
					hud.ProgressStateTime=0
					itemScale=.66
				end
			end
			...
			if (hud.ProgressPreviousState~=iCurrState and hud.ProgressPreviousState~=-1 and iCurrState>0) then
				hud.ProgressStateTime=1
			end
			if (iCurrState==0) then
				hud:DrawQuadMP(x,y,.66,.66,100,hud.tmultiplayerhud.progressi_changing,1,1,1,1,0,0)
		...
		elseif (iItemCurrent < tonumber(PTable.ValueX)) then
			hud:DrawQuadMP(x,y,.66,.66,100,hud.tmultiplayerhud.progressi_done,1,1,1,1,0,0)
		else
			hud:DrawQuadMP(x,y,.66,.66,100,hud.tmultiplayerhud.progressi_unavailable,1,1,1,1,0,0)
		end
...
function Hud:SetRadarObjective(tagPointName)
   ...
   Hud.RadarObjectiveName = tagPointName
...
function Hud:ProcessAddMessage(tMsgList,text,lifetime,beep,killmsg)
	...
	if (tMsgList and tMsgList[count(tMsgList)] and  tMsgList[1].lifetime) then
		if (tMsgList[1].lifetime>=6) then
			lifetime=tMsgList[1].lifetime+.01
		end
	end
...
-----
-- enter new messages to messages box
-----
...
function Hud:OnSave(stm)
	...
	if (self.RadarObjectiveName==nil) then
		stm:WriteString("nil")
	else
		stm:WriteString(self.RadarObjectiveName)
		-- WriteToStream(stm,self.RadarObjectiveName)
	end
...
function Hud:OnLoad(stm)
	...
	-- local sObj=stm:ReadString()
	-- if (sObj) then
		-- self.RadarObjectiveName=ReadFromStream(stm)
		self.RadarObjectiveName=stm:ReadString()
	-- end
...
function Hud:CommonInit()
	...
	if getglobal("g_language") and strlower(getglobal("g_language"))=="russian" then
		Language:LoadStringTable("HUD_ru.xml")
	else
		Language:LoadStringTable("HUD_en.xml")
	end
	...
	self.icon_team_reload=System:LoadImage("Textures/SpecOps/reload_team.dds") -- SpecOps
	...
	-- self.Radar2=System:LoadImage("Textures/hud/compass.dds")
	...
	if (cur_r_TexResolution >= 1) then -- lower res texture for low/med texture quality setting
		self.TID_Compass=System:LoadImage("Textures/Hud/Binocular/binoculars_compass_low.tga")
	else
		self.TID_Compass=System:LoadImage("Textures/Hud/Binocular/binoculars_compass.tga")
	end
	...
	Game:CreateVariable("hud_fadeamount",1) -- Нерозрачность хада. Изменяет всё, кроме радара. Исправить.
	self.blinding = 0
	...
	local s={
		...
		{x=sx+sw/2,y=sy,u=.5,v=0},
		{x=sx+sw/2,y=sy+sh,u=.5,v=1},
		{x=sx+sw-1,y=sy,u=1,v=0},
		{x=sx+sw-1,y=sy+sh,u=1,v=1},
	}
	local h={
		{x=11+1,y=525,u=0,v=0},
		{x=10+1,y=557,u=0,v=.5},
		{x=42,y=525,u=.5,v=0},
		{x=42,y=557,u=.5,v=.5},
		{x=74-1,y=525,u=1,v=0},
		{x=74-1,y=557,u=1,v=.5},
	}
	local a={
		{x=10+1,y=557,u=0,v=.5},
	...
	Game:CreateVariable("g_ConcentrationAmbientVolume",.7)
	Game:CreateVariable("g_ConcentrationMissionVolume",1)
	Game:CreateVariable("g_ConcentrationNormalVolume",.9)
...
function Hud:InitTexTableMP(tt)
	local tw,th=256,128
	local offsetU,offsetV=2/256,2/128
...
function Hud:DrawQuadMP(x,y,scalex,scaley,val,texi,r,g,b,a,flipu,flipv)
	...
	if (fValue~=1) then
		self.mp_rend:PushQuad(x-texi.size.w*scalex*.5,y-texi.size.h*scaley*.5,texi.size.w*scalex,texi.size.h*scaley,texi,r,g,b,fValue)
	else
		self.mp_rend:PushQuad(x-texi.size.w*scalex*.5,y-texi.size.h*scaley*.5,texi.size.w*scalex,texi.size.h*scaley,texi,r,g,b,a)
	end
end
...
----------------------------------
-- enter a pickup to hud pickup list
----------------------------------
...
function Hud:AddPickup(pick_type,pick_amount)
	...
	if (self.pPickupsTbl and self.pPickupsTbl[count(self.pPickupsTbl)] and self.pPickupsTbl[1].Lifetime) then
		if (self.pPickupsTbl[1].Lifetime>=1) then
			lifetime=self.pPickupsTbl[1].Lifetime+.01
		end
	end

	-- then enter pickup to list
		...
		for i,Pickup in self.pPickupsTbl do
			if (Pickup.Lifetime>0) then
				...
				if (Pickup.Amount==-1) then
					-- not available
					self:DrawElement(720-Pickup.Position,500,self.pickups[19], 1,1,1,fLifetime*.9)
				...
				Pickup.Lifetime=Pickup.Lifetime-_frametime*.25
...
function Hud:DrawItems(player) -- Сделав другой компасс нужно будет переместить чуть ниже.
	local x=400
	local y=510
...
function Hud:DrawLabel()
	if (self.label) then
		...
		%Game:WriteHudString(400-strsizex*.5,350,self.label,self.color.r,self.color.g,self.color.b,1,12,12,0)
	end
...
function Hud:DrawEnergy(player)
	...
	local stamina=self.staminalevel*38.5
	if (stamina<0) then
		stamina=0
	elseif (stamina>255) then
		stamina=255
	end
	...
	if (self.curr_stamina<0) then
		self.curr_stamina=0
	elseif (self.curr_stamina>255) then
		self.curr_stamina=255
	end
	...
	if (self.curr_health>.01) then
		-- display energy bars
		if (health<30) then
			self.curr_healthAlpha=self.curr_healthAlpha+FrameTime*2
			if (self.curr_healthAlpha>1) then
		...
		--if (self.bBlinkEnergy==0 or self.fBlinkEnergyUpdate>.5) then
			self:DrawGauge(583,546,self.curr_health,100,self.txi.health_inside,.7*.709,.7*.219,.7*.233,.9*self.curr_healthAlpha,0,0) -- Если закомить, уберётся плавность. http://www.farcry.ru/forum/viewtopic.php?t=1533&postdays=0&postorder=asc&start=140

			self.vColor.r=.709+self.fBlinkEnergyUpdate
			self.vColor.g=.219+self.fBlinkEnergyUpdate
			self.vColor.b=.233+self.fBlinkEnergyUpdate

			-- clamp color to 1
			if (self.vColor.r>1) then self.vColor.r=1  end
			if (self.vColor.g>1) then self.vColor.g=1  end
			if (self.vColor.b>1) then self.vColor.b=1  end

			self:DrawGauge(583,546,health,100,self.txi.health_inside,self.vColor.r,self.vColor.g,self.vColor.b,.9*self.curr_healthAlpha,0,0)
		--end
	end
	...
	if (self.curr_armor>.01) then
		--if (self.bBlinkArmor==0 or self.fBlinkArmorUpdate>.5) then
			self:DrawGauge(572,561,self.curr_armor,100,self.txi.health_inside,.7*.776,.7*.541,.7*.258,.9,0,0) -- И это тоже.

			self.vColor.r=.776+self.fBlinkArmorUpdate
			self.vColor.g=.541+self.fBlinkArmorUpdate
			self.vColor.b=.258+self.fBlinkArmorUpdate
			-- clamp color to 1
			if (self.vColor.r>1) then self.vColor.r=1  end
			if (self.vColor.g>1) then self.vColor.g=1  end
			if (self.vColor.b>1) then self.vColor.b=1  end

			self:DrawGauge(572,561,armor,100,self.txi.health_inside,self.vColor.r,self.vColor.g,self.vColor.b,.9,0,0)
		--end
	end

	if (self.curr_stamina>.01) then
		self:DrawGauge(560,576,self.curr_stamina,100,self.txi.health_inside,.258,.49,.807,.9,0,0) -- А это тоже?
	end
...
function Hud:DrawAmmo(player)
	...
	if (player.fireparams and player.fireparams.bullets_per_clip) then
		critical_ammo_count=(player.fireparams.bullets_per_clip*.3)
	end
...
function Hud:DrawCompass()
	 local u=(-_localplayer:GetAngles().z+217)/360+.27
	-- Картинка, "x" позиция, "у" позиция, ширина, высота, "тип чего-то там смешанного"(blend type) - плотность что-ли, r,g,b, alpha, , , , , .
	-- System:DrawImageColorCoords(self.TID_Compass,340,585,100,20,4,1,1,1,1,u-.04,1,u+.04,0)
	System:DrawImageColorCoords(self.TID_Compass,350,585,100,20,5,1,1,1,1,u-.04,1,u+.04,0)
end

function Hud:DrawRadar(x,y,w,h,newcompass)

	if (hud_disableradar=="1") then
		return
	end

	if tonumber(getglobal("game_DifficultyLevel"))<4 then
	...
	-- Меняет масштаб радара внутри помещений/снаружи.
	...
	-- Сделать таблицу игроков пустой.
	for i,Player in Hud.tblPlayers do
		Hud.tblPlayers[i] = nil
	end
	...
	if (Hud.tblPlayers and type(Hud.tblPlayers)=="table") then
		for i,Player in Hud.tblPlayers do
			if (Player.pEntity==LocalPlayer) then -- is player ?
				Hud:SetPlayerColor(Player,0,.9,.8,Hud.radar_transparency) -- Похоже, сам игрок.
			elseif (Player.pEntity.Properties.species==LocalPlayer.Properties.species) then	--is enemy of friend ? Друзья игрока.
					  -- radar fade in
		          	local alpha_step=1
		          	Hud.radar_transparency = Hud.radar_transparency + alpha_step*_frametime
		          	if (Hud.radar_transparency>1) then
		            	Hud.radar_transparency=1
		          	end
					...
					if (Game:IsMultiplayer()) then
						...
						if (team and LocalPlayerTeam and (self.sGameType~="FFA")) then
							...
							if LocalPlayerTeam~=team and not Player.pEntity.ShowOnRadar then
								Hud.tblPlayers[i]=nil
							end
						else
							-- then only display entities tagged by motion tracker
							if (Player.pEntity.ShowOnRadar) then
						...
						--if (Player.pEntity.ShowOnRadar) then
						local transparency = Hud.radar_transparency
						if tonumber(getglobal("game_DifficultyLevel"))>= 4 or Player.pEntity.bEnemy_Hidden or Player.pEntity.IsInvisible then
							transparency=0
						end
						Hud:SetPlayerColor(Player,1,.8,1,transparency)
						--end
					end
			elseif Player.pEntity.Properties.bAffectSOM==1 and not Player.pEntity.bEnemy_Hidden and not Player.pEntity.IsInvisible and tonumber(getglobal("game_DifficultyLevel"))<4 then -- Сделать исчезновение с радаров плавным.
				if (Player.pEntity.ShowOnRadar and Player.pEntity.ShowOnRadar==3) then -- Чтобы зачистить список. Только нужно сделать переменную из булева типа в другую.
					Player.pEntity.ShowOnRadar=nil
				end
				...
				if Player.pEntity.ShowOnRadar then
					...
					if Player.pEntity.bEnemyInCombat==1 or Player.pEntity.WasInCombat then -- Сделать чтобы постепенно при возвращении в идл менялся и цвет.
						Hud:SetPlayerColor(Player,1,0,0,Hud.radar_transparency)
					else
						...
						if TargetType=="table" then -- Не станет красным, пока не добавится в бинокль.
							Hud:SetPlayerColor(Player,1,0,0,Hud.radar_transparency) -- combat Бой
							self.EnemyAlerted = 1
							Player.pEntity.bEnemyInCombat = 1  	-- enemy saw player Враг помнит врага
						elseif TargetType=="number" then
						 	if Target==AIOBJECT_DUMMY then
						 		if (Player.pEntity.Behaviour.JOB==nil) then
									Hud:SetPlayerColor(Player,1,.5,0,Hud.radar_transparency)	-- threatened Находится под угрозой
									self.EnemyAlerted = 1
								else
									Hud:SetPlayerColor(Player,0,1,0,Hud.radar_transparency)	-- doing something Что-то делает
								end
							...
							-- no target ?
							if (Player.pEntity.Behaviour.JOB==nil) then
								Hud:SetPlayerColor(Player,1,1,0,Hud.radar_transparency) -- heared something Что-то услышал
							else
								Hud:SetPlayerColor(Player,0,1,0,Hud.radar_transparency) -- doing something
							end
						end
		...
		if (Hud.radarObjective and (self.bBlinkObjective==0 or self.fBlinkObjectiveUpdate>.5)) then
		...
		-- end
		-- render radar
		Game:DrawRadar(x,y,w,h,tonumber(g_RadarRange),radar,self.RadarMask,self.RadarPlayerIcon,
...
function Hud:DrawStealthMeter(x,y)
  	...
  	if (self.DisplayControl.bShowEnergyMeter==0 or self.DisplayControl.bShowRadar==0 or self.DisplayControl.fBlinkUpdateRadar>.5) then
    	return
   	end
	...
	self:DrawElement(4-1,522-6,self.txi.shape_stealth_left,1,1,1,.9)
	self:DrawElement(91.5,522-6,self.txi.shape_stealth_right,1,1,1,.9)

	if (self.curr_stealth>.01) then
		self:DrawGauge(6-1,523-6,100,self.curr_stealth,self.txi.sealth_inside_left,1,1,1,.75,0,0)
		self:DrawGauge(94.5,523-6,100,self.curr_stealth,self.txi.sealth_inside_right,1,1,1,.75,0,0)
	end
end
...
function Hud:DrawDamageIndicator(player) -- Должно быть всегда, человек же чуствует где больно...
	-- Индикатор попаданий по мне (красная рамка).
	-- if tonumber(hud_damageindicator)~=0 then
		...
		if (FrameTime<.002) then
			FrameTime=.002
		elseif (FrameTime>.5) then
			FrameTime=.5
		end

		FrameTime=FrameTime*.75
		local fTexOffset=.5/256

		if (self.dmgfront>0) then
			%System:DrawImageColorCoords(self.damage_icon_ud,0,0,800,90,4,.45,.1,0, self.dmgfront,1+fTexOffset,1+fTexOffset,fTexOffset,fTexOffset)
			self.dmgfront=self.dmgfront-FrameTime
			if (self.dmgfront<0) then self.dmgfront=0 end
		end

		if (self.dmgback>0) then
			%System:DrawImageColorCoords(self.damage_icon_ud,0,600-90,800,90,4,.45,.1,0,self.dmgback,fTexOffset,fTexOffset,1+fTexOffset,1+fTexOffset)
			self.dmgback=self.dmgback-FrameTime
			if (self.dmgback<0) then self.dmgback=0 end
		end

		if (self.dmgleft>0) then
			%System:DrawImageColorCoords(self.damage_icon_lr,0,0,90,600,4,.45,.1,0,self.dmgleft,fTexOffset,fTexOffset,1-fTexOffset,1-fTexOffset)
			self.dmgleft=self.dmgleft-FrameTime
			if (self.dmgleft<0) then self.dmgleft=0 end
		end
		if (self.dmgright>0) then
			%System:DrawImageColorCoords(self.damage_icon_lr,800-90,0,90,600,4,.45,.1,0,self.dmgright,1-fTexOffset,1-fTexOffset,fTexOffset,fTexOffset)
...
function Hud:DrawCrosshair(player)
		-- На сложностях четыре перекрестья нет.
		...
		if w then
			...
			Hud.hit=Hud.hit-20*_frametime -- Убирает покраснение при попадании по целям.
		end
		self.currCrossAirScale=self.currCrossAirScale-_frametime*10
		if (self.currCrossAirScale<1) then
			self.currCrossAirScale=1
		end
end
...
function Hud:DrawVehicleBar(vehicle,health)
	if (health~=0 and (vehicle.IsCar or vehicle.IsBoat)) then
		...
		if (healthStep<45) then
			...
			if (self.curr_vehicledamageAlpha>1) then
...
function Hud:MissionBox()
	...
	for i,val in self.objectives do
		if (not val.completed) then
			...
			%System:DrawImageColor(self.missioncheckboxcomp,boxx+textspacing,y,16,16,4,.35,.35,.39,1)
			%Game:WriteHudString(boxx+textspacing+20,y-3,val.text,.35,.35,.39,1,20,20)
		end
		y=y+30
	end
end
...
function Hud:DrawFrameBox(x,y,w,h,a) -- икс, игрик, ширина, высота, прозрачность.
	...
	if (n>0) then
		...
		for i,msg in tMsgList do
			if (msg.lifetime>0) then
				...
				-- fade out old msg's
				local textalpha=(4*msg.lifetime*msg.lifetime*msg.lifetime)/6
...
function Hud:MessagesBox()
	...
	if Difficulty>=4 then
		self:DrawMessagesBox(self.messages,8,580)
	else
...
function Hud:ResetSubtitles()
	...
	Hud.fSubtlCurrY=0
	Hud.fSubtlCurrDelay=0
end
-----
-- enter new messages to subtitles box
-----
...
function Hud:DrawSubtitlesBox(tMsgList,xpos,ypos)
	...
	if (n>0) then
		...
		-- render subtitle box
		-- self:DrawFrameBox(20,20-9-4,800-40,fFinalBoxHeight,.5)
		self:DrawFrameBox(0,0,1000,fFinalBoxHeight,.2) -- Альфа 0 - прозрачный.
		self:FlushCommon()

		-- set scissoring area
		-- System:SetScissor(20,20-9,800-40,fFinalBoxHeight-8)
		System:SetScissor(0,0,1000,fFinalBoxHeight)
		...
		for i,msg in tMsgList do
			if (msg) then
				...
				--if (msg.lifetime>0) then
					...
					-- just write text
					-- px, py, sStringKey, r, g, b, a, bxsize(жирность), ysize(размер), bCenter(по центру экрана), fWrapWidth (максимальная ширина, если ноль, то неограничена)
					-- Game:WriteHudString(400-strsizex*.5,ypos+y-Hud.fSubtlCurrY,msg.text,0,.75,1,textalpha,14,14,0,maxFrameBoxSize)
					Game:WriteHudString(400-strsizex*.5,ypos+y-Hud.fSubtlCurrY+8,msg.text,0,.75,1,textalpha,14,14,0,maxFrameBoxSize)
					-- Game:WriteHudString(310-strsizex*.5,ypos+y-Hud.fSubtlCurrY+8,msg.text,0,.75,1,textalpha,14,14,0,maxFrameBoxSize+100)
					y= y + strsizey+8
		...
		if (boxHeight-fmaxBoxSizey>0) then
			-- activate fake delay..
			if (Hud.fSubtlCurrDelay>0 and _time-Hud.fSubtlCurrDelay>1) then
		    if (boxHeight-fmaxBoxSizey>Hud.fSubtlCurrY) then
					Hud.fSubtlCurrY=Hud.fSubtlCurrY+3*_frametime  -- Скорость прокрутки текста по вертикали => до исчезновения субтитров. -- 3
				else
...
function Hud:SubtitlesBox()
	...
	if (subCount>=1) then
		...
		self:DrawSubtitlesBox(self.tSubtitles,0,0) -- x,y
	end
end
...
function Hud:DrawWeaponSlots(player)
	...
	if (Hud.weapons_alpha>0) then
		...
		for i,val in weapons do
			if self.curr_weapon==9 -- Руки.
			or self.curr_weapon==23 -- Mortar.
			or self.curr_weapon==25 -- MG.
			or self.curr_weapon==31 -- VehicleMountedMG.
			or self.curr_weapon==32 -- VehicleMountedRocketMG.
			or self.curr_weapon==33 -- VehicleMountedRocket.
			or self.curr_weapon==34 -- VehicleMountedAutoMG.
			then break end
			if (val and type(val)=="table") then
				local currId=Game:GetWeaponClassIDByName(val.name)
				if currId~=self.curr_weapon then
					if x<=SaveX+84*3 then
						self:DrawWeapon(x,y,currId,.4*alpha_scale)
						x=x+84 -- На всякий случай.
					end
				else
					alpha_scale=alpha_scale*2
					if (alpha_scale>1) then
						alpha_scale=1
					end
					if x<=SaveX+84*3 then
				...
				if x<=SaveX+84*3 then
					self:DrawWeapon(x,y,0,.4*alpha_scale)
					x=x+84
				end
				-- System:Log("X2 = "..x)
			end
		...
		local FrameTime=_frametime
		Hud.weapons_alpha=Hud.weapons_alpha-(FrameTime*.25)
		if (Hud.weapons_alpha<0) then
			Hud.weapons_alpha=0
		end
	end
...
function Hud:DrawWeapon(x,y,w,alpha)
	...
	self:DrawElement(x,y,self.txi.shape_bar,1,1,1,.5*alpha)
	if (w and w and self.tweapons[w]) then
		local txi=self.txi.shape_bar
		local weapon_txi=self.tweapons[w]
		local offsetX,offsetY= txi.size.w*.5,txi.size.h*.5

		offsetX=offsetX-weapon_txi.size.w*.5
		offsetY=offsetY-weapon_txi.size.h*.5
...
function Hud:OnMeleeDamage(damage_type)
	--System:Log("client damage "..damage_type)
	if (damage_type) then
...
function Hud:DrawGooglesOMeter(x1,y1)
	...
	if (LocalPlayer.items and LocalPlayer.items.heatvisiongoggles) then
		if (LocalPlayer.Energy~=0) then
			...
			self:DrawGauge(x+4,y+4,LocalPlayer.Energy,100,self.txi.googles_energy_inside,1,1,1,.9,0,0)
...
-------------------------------------
function Hud:OnUpdateCommonHudElements() -- Разобраться,почему пропадет счетчик патронов
	local player = _localplayer
	-- Рисовать прицел.
	if hud_crosshair=="1" then
		self:DrawCrosshair(player)
	end
	-- Ничего не показывать, если игрок - наблюдатель.
	if (player.entity_type=="spectator") or player.cnt.health==0 then do return end end
	local wscope = ClientStuff.vlayers:IsActive("WeaponScope")
	local w = _localplayer.cnt.weapon
	local Difficulty = tonumber(getglobal("game_DifficultyLevel"))

	-- Работает в режиме разработчика.
	-- if (not self.thirdRange) then --first set up of variable
            -- self.thirdRange=5
            -- self.thirdAngle=45
            -- Game:SetThirdPerson(1) --let's go in third person !
        -- end
        -- local DownKey=Input:GetXKeyDownName() --key down
	-- if (DownKey==".") then --if you press c
		-- if (self.thirdRange>.01) then
			-- self.thirdRange=self.thirdRange-System:GetFrameTime()*20  --distance from the camera gets lower
			-- Game:SetVariable("ThirdPersonRange",self.thirdRange)
		-- end
	-- elseif (DownKey=="/") then --if you press c
		-- if (self.thirdRange<50) then
			-- self.thirdRange=self.thirdRange+System:GetFrameTime()*20 --distance from the camera gets higher
			-- Game:SetVariable("ThirdPersonRange",self.thirdRange)
		-- end
	-- elseif (DownKey==' ') then --if you press b
		-- if (self.thirdAngle>.1) then
			-- self.thirdAngle=self.thirdAngle-System:GetFrameTime()*20
			-- Game:SetVariable("ThirdPersonAngle",self.thirdAngle)
		-- end
	-- elseif (DownKey=="'") then --if you press n
		-- if (self.thirdAngle<360) then
			-- self.thirdAngle=self.thirdAngle+System:GetFrameTime()*20
			-- Game:SetVariable("ThirdPersonAngle",self.thirdAngle)
		-- end
    -- end
	...
	if FPS and System:IsDevModeEnable() then
		if not Timer or _time>Timer+.1 then Timer = _time UpdateFps=FPS end
		%Game:WriteHudString(715,10,"FPS: "..UpdateFps,0,1,0,1,30,30)
	end

	if player.cnt.underwater > .06 then -- Получается эффект размытия. http://verysoft.narod.ru/fcbugfix.htm
		if self.hitdamagecounter < 6.5 then
			Hud:SetScreenDamageColor(.5, .5, .6)
			self.hitdamagecounter = 6.5
		end
	elseif UI then -- Только в игре.
	-- else
		-- Холодный - .2, 0, .1, .2, Зелёноватый - .2, .2, .3, .2, чуть побольше - .2, .3, .4, .3.
		local Amout = tonumber(getglobal("game_ScreenBlurAmout"))
		if not Amout then Amout = .2 Game:CreateVariable("game_ScreenBlurAmout",Amout)	end
		-- if ClientStuff.vlayers:IsActive("NightVision") or ClientStuff.vlayers:IsActive("HeatVision") then Amout = 0 end
		local R = tonumber(getglobal("game_ScreenBlurR"))
		if not R then R = .3 Game:CreateVariable("game_ScreenBlurR",R)	end
		local G = tonumber(getglobal("game_ScreenBlurG"))
		if not G then G = .4 Game:CreateVariable("game_ScreenBlurG",G)	end
		local B = tonumber(getglobal("game_ScreenBlurB"))
		if not B then B = .3 Game:CreateVariable("game_ScreenBlurB",B)	end
		System:SetScreenFx("ScreenBlur",1)
		System:SetScreenFxParamFloat("ScreenBlur","ScreenBlurAmount",Amout)
		Hud:SetScreenDamageColor(R,G,B)
	end

	-- Красная рамка по бокам.
	self:DrawDamageIndicator(player)
	...
	-- -- Выключить датчик движения в бинокле, если игрок захочет включить его на высоких уровнях сложности.
	-- if tonumber(getglobal("cl_motiontracker"))==1 and Difficulty>=4 then
		-- setglobal("cl_motiontracker",0)
	-- elseif tonumber(getglobal("cl_motiontracker"))==0 and Difficulty<4 then
		-- setglobal("cl_motiontracker",1)
	-- end
	-- Мигающий значок в правом нижнем углу от датчика движения в бинокле.
	if tonumber(getglobal("cl_motiontracker"))==1 and Difficulty<4 then
		if (ClientStuff and ClientStuff.vlayers and ClientStuff.vlayers:IsActive("Binoculars")) then
			...
			if (self.curr_motiontrackerAlpha>1) then
				self.curr_motiontrackerAlpha=0
			end
			self:DrawElement(765,470,self.txi.motiontracker_border,1,1,1,1)
			self:DrawElement(765+(30-14)*.5,470+(24-14)*.5,self.txi.motiontracker_signal,1,1,1,self.curr_motiontrackerAlpha)
		end
	end

	-- Общее фоновое затемнение.
	self:FlushCommon()

	if ClientStuff.vlayers:IsActive("Binoculars") or (w and wscope and not _localplayer.cnt.drawfpweapon) then return end
	--------------
	-- blink radar
	...
	if (self.DisplayControl.bShowRadar==1) then
		if (self.DisplayControl.bBlinkRadar==0 or self.DisplayControl.fBlinkUpdateRadar>.5) then
			if Difficulty>=4 then
				self:DrawRadar(350,548,104,102,1)
			else
	...
	Hud:DrawCrosshairName() -- Отображать имена союзников.

	-- Показывать подсказки типа: "Нажмите x чтобы сделать..."
	self:DrawLabel()

	-- Если игрок садится в машину,то показывать состояние этой машины.
	local vehicle=player.cnt:GetCurVehicle()
	if (vehicle) then
		self:DrawVehicleBar(vehicle,vehicle.cnt.engineHealthReadOnly)
	end

	-- Показывать слоты оружия (осветление здесь не используется).
	if (self.DisplayControl.bShowWeapons==1) then -- Проверить звуки при смене и придумать что-нибудь со звуками выбора гранат.
		self:DrawWeaponSlots(player)
	end

	-- Показывать подобранные "призы".
	if (cl_hud_pickup_icons=="1") then
		Hud:DrawPickups()
	end

	-- Показывать уровень заряда батарей от прибора ночного виденья.
	...
	if (self.DisplayControl.bShowEnergyMeter==1) then
		if (self.DisplayControl.bBlinkEnergyMeter==0 or self.DisplayControl.fBlinkUpdateEnergyMeter>.5) then
	...
	if (self.DisplayControl.bShowAmmo==1) then
		if (self.DisplayControl.bBlinkAmmo==0 or self.DisplayControl.fBlinkUpdateAmmo>.5) then
