...
Idle_Any = {
	...
	RemoveAttachments = function(self,entity)
		...
		if (self.Attachment) then
			for i,value in self.Attachment do
	...
	OnSpawn = function(self,entity)
		entity.ActivateJobAnimation = nil
	...
	ANCHOR_REACHED = function(self,entity,sender)
		if (self.AFFECT_POSITION) then
			...
			local approach_target = AI:FindObjectOfType(entity:GetPos(),3,self.ANCHOR_TO_APPROACH,pos)
		...
		if (self.BaseLoopAnimation) then
			...
			AI:EnablePuppetMovement(entity.id,0,anim_table[4])
		end
		...
		if (self.StartAnimation) then
			entity.ActivateJobAnimation = 1 -- Теперь ещё и CryGame
			local anim_table = self:GetOneAnimation(self.StartAnimation,entity)
			entity:InsertAnimationPipe(anim_table[1],3,"START_ANIM_FINISHED",anim_table[4],nil,1)
	...
	PLAY_BREAK_ANIMATION = function(self,entity,sender)
		if (self.BreakAnimation) then
			...
			entity:InsertAnimationPipe(anim_table[1],3,BREAK_ANIM_FINISHED,anim_table[4],nil,1)
		end
		...
		if (self.NrBreaks) then
			if (self.NrBreaks > 0) then
				...
				if (self.EndAnimation) then
					...
					entity:InsertAnimationPipe(anim_table[1],3,END_ANIM_FINISHED,anim_table[4],nil,1)
					if (entity.AI_ParticleHandle) then
						Particle:Detach(entity.id,entity.AI_ParticleHandle)
					end
				end
