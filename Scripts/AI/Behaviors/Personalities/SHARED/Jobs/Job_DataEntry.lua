...
AIBehaviour.Job_DataEntry = {
	...
	FIND_CHAIR = function(self,entity)
		if (entity.AI_Chair==nil) then
			entity.AI_Chair= AI:FindObjectOfType(entity.id,10,AIAnchor.AIANCHOR_CHAIR)
		end
		
		if (entity.AI_Chair) then
--			System:Log("\001["..entity:GetName().."] Job_DataEntry+++++++++++++++FIND_CHAIR found chair")
	...
	FIND_ANCHOR = function(self,entity)
		...
		if (random(1,2)==1) then 
			entity.AI_TargetType = AIAnchor.AIANCHOR_SIT_WRITE 
		else
			entity.AI_TargetType = AIAnchor.AIANCHOR_SIT_TYPE 
		end
		...
		if (entity.AI_FoundObject) then
			entity:SelectPipe(0,"bindChair_trace",entity.AI_FoundObject)
		         ...
		         if (entity.AI_TargetType==AIAnchor.AIANCHOR_SIT_WRITE) then
			entity.AI_TargetType = AIAnchor.AIANCHOR_SIT_TYPE 
		         else
		       	 entity.AI_TargetType = AIAnchor.AIANCHOR_SIT_WRITE 
		         end
	...
	SITDOWN_ANIM = function(self,entity,sender)
		entity:StartAnimation(0,self.AnimTable[AIAnchor.AIANCHOR_CHAIR][random(1,2)])
	end,
	...
	SITUP_ANIM = function(self,entity,sender)
		entity.cnt.AnimationSystemEnabled = 1 
		entity:StartAnimation(0,self.AnimTable[AIAnchor.AIANCHOR_CHAIR][3])
	end,
	---------------------------------------------
	...
	BIND_CHAIR_TO_ME = function(self,entity,sender)
		...
		local chairName = AI:FindObjectOfType(entity.id,3,AIAnchor.AIOBJECT_SWIVIL_CHAIR)
		if (chairName) then
			entity.AI_BoundObject = System:GetEntityByName(chairName)--see ScriptObjectSystem

			if (entity.AI_BoundObject) then
				entity.AI_BoundObject:SetPos({x = 0,y = .5,z = 0})
