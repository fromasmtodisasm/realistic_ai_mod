...
AIBehaviour.Job_SitDown = {
	...
	JOB = 1,
	AnimTable = {
		[AIAnchor.AIANCHOR_MAGAZINE] = {"magazine1","magazine2","magazine3"},
		[AIAnchor.AIANCHOR_CHAIR] = {"sitdown","sitdown_legup","situp"},
		none = {"sitdown_breath"},
	},
	TargetType = AIAnchor.AIANCHOR_MAGAZINE,
	--------------------------
	...
	BIND_CHAIR_TO_ME = function(self,entity,sender)
		local chairName = AI:FindObjectOfType(entity.id,5,AIAnchor.AIOBJECT_SWIVIL_CHAIR)
		if (chairName) then
			entity.AI_BoundObject = System:GetEntityByName(chairName)--see ScriptObjectSystem
			if (entity.AI_BoundObject) then
				entity.AI_BoundObject:SetPos({x = 0,y = .6,z = 0})
