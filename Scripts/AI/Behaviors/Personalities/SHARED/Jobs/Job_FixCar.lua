...
AIBehaviour.Job_FixCar = {
	...
	JOB = 1,
	AnimTable =		{
					[AIAnchor.AIANCHOR_WHEEL] = {"_fixwheel_start","_fixwheel_loop","_fixwheel_end"},
				 	[AIAnchor.AIANCHOR_HOOD] = {"_fixfence_start","_fixfence_loop","_fixfence_end"},
				},
	TargetType = AIAnchor.AIANCHOR_WHEEL,
	------------------------------------------------------------------------ 	
	...
	FIND_ANCHOR = function (self,entity)
		...
		if (entity.AI_TargetType and (entity.AI_TargetType == AIAnchor.AIANCHOR_HOOD)) then
			entity.AI_TargetType = AIAnchor.AIANCHOR_WHEEL;
		else
			if (random(1,2) == 1) then 
				entity.AI_TargetType = AIAnchor.AIANCHOR_HOOD;
			else
				entity.AI_TargetType = AIAnchor.AIANCHOR_WHEEL;
			end
		end
		...
		if (entity.AI_FoundObject) then
			...
			if (entity.AI_TargetType == AIAnchor.AIANCHOR_WHEEL) then
	...
	DECISION_POINT = function( self,entity , sender)
		...
		elseif ( (rnd > 6) and (entity.AI_TargetType) and (entity.AI_TargetType == AIAnchor.AIANCHOR_WHEEL)) then		
	...
	GOT_TOOLS = function( self,entity , sender)
		...
		entity:StartAnimation(0,self.AnimTable[AIAnchor.AIANCHOR_HOOD][1]);
		entity:SelectPipe(0,"anchor_animation",entity.AI_FoundObject);
		entity:InsertSubpipe(0,"pause");
