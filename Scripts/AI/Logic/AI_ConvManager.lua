...
function AI_ConvManager:GetSpecificConversation(name,conv_id)
	local conv
	if name=="RANDOM" then
		conv = AI_IdleConversations[conv_id]
	else
		conv = AI_CriticalConversations[name][conv_id]
	end
	self.NewConversation = new(AI_Conversation)
	self.NewConversation.Participants = conv.Participants
	self.NewConversation.InPlace = conv.InPlace
	self.NewConversation.Joined = 0
	self.NewConversation.InPlaceJoined = 0
	self.NewConversation.Progress = 0
	...
	System:Log("GetSpecificConversation")
...
function AI_ConvManager:GetControlledRandomConv(conv_table)
	...
	conv.UseTag = 1
	conv.CONV_ID = rnd
	-- System:Log("GetControlledRandomConv")
	return conv
end

function AI_ConvManager:GetRandomIdleConversation(InPlace)
	...
	if InPlace or self.NewConversation.InPlace then
...
function AI_ConvManager:GetRandomCriticalConversation(name,InPlace)
	...
	self.NewConversation.InPlace = RandomConv.InPlace
	self.NewConversation.Joined = 0
	self.NewConversation.InPlaceJoined = 0
	self.NewConversation.Progress = 0
